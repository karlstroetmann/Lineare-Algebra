...
\begin{Definition}[Identische Relation]
  ...
  wie folgt: \\[0.2cm]
  \hspace*{1.3cm} $\id_M := \bigl\{ \pair(x,x) \mid x \in M \bigr\}$.  \eox
\end{Definition}

\example
Es sei $M = \{1,2,3\}$.  Dann gilt \\[0.2cm]
\hspace*{1.3cm}  $\id_M := \bigl\{ \pair(1,1),  \pair(2,2),  \pair(3,3) \bigr\}$.  \eox
\vspace*{0.2cm}

\remark
Aus der Definition folgt sofort \\[0.2cm]
\hspace*{1.3cm} $\id_M^{-1} = \id_M$. \eox
...
\exercise
Eine Relation $R \subseteq M \times M$ ist \emph{asymmetrisch} genau dann, wenn $R$ nicht
symmetrisch ist.  
\begin{enumerate}
\item Geben Sie eine Menge $M$ und eine Relation $R \subseteq M \times M$ an, so dass $R$
      anti-symmetrisch, aber nicht asymmetrisch ist.
\item Geben Sie eine Menge $M$ und eine Relation $R \subseteq M \times M$ an, so dass $R$
      asymmetrisch, aber nicht anti-symmetrisch ist. \eox
\end{enumerate}
...
\begin{Definition}[transitiver Abschluss, $R^+$]
  ...
  \hspace*{1.3cm} $\bigcup\limits_{i=1}^{\infty} A_n = A_1 \cup A_2 \cup A_3 \cup \cdots $. \eox
\end{Definition}
\pagebreak
...
zusammen mit der Relation $\approx_n$, die wir für natürliche Zahlen wie folgt definieren: \\[0.2cm]
\hspace*{1.3cm}
...
die diese Äquivalenz-Relation erzeugt. \eox
...
\\[0.2cm]
\hspace*{1.3cm}
$x \approx_n y \;\leftrightarrow\; x \modulo n = y \modulo n$. \eox
...
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x,y) \in R \;\leftrightarrow\; [x]_R = [y]_R$. \eox
