...
\begin{center}
\emph{Eine \emph{Menge} ist eine \emph{wohldefinierte} Ansammlung von \emph{Elementen}.}    
\end{center}
...
als natürliche Zahl auffassen. 
  Mit den bisher behandelten Verfahren läßt sich die Menge $\mathbb{N}$  nicht definieren.
...
\item $\mathbb{R}$: Menge der reellen Zahlen.
      ...
      Wie die reellen Zahlen tatsächlich konstruiert werden, kann ich Ihnen erst im
      zweiten Semester zeigen.
\end{enumerate}
...
Diese Menge hat $8 = 2^3$ Elemente.  Allgemein kann durch Induktion über die Anzahl der
...
die in den folgenden Gleichungen zusammengefaßt sind:
\\[0.2cm]
$\begin{array}{rlcl}
...
Abschlusses einer Relation intuitiver wird.  \qed
...
\hline
  \end{tabular} 
\end{center}
Diese Relation könnte dann zur Rechnungsstellung weiter verwendet werden. \qed

\noindent
\textbf{Bemerkung}: In der Theorie der Datenbanken wird der Begriff der Komposition zweier
Relationen zu dem Begriff des \emph{Joins} verallgemeninert.
...
$\pair(x,y) \in R$ auch $x\, R\, y$.  Beispielsweise läßt sich die Relation $\leq$ 
...
\noindent
\textbf{Beweis}:  Wir beweisen die Behauptungen in der selben Reihenefolge wie oben angegeben.
\begin{enumerate}
