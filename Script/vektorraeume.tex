...
\emph{Skalare} genannt werden.  Es ist üblich, Vektoren von Skalaren durch Fettdruck zu
unterscheiden.  Da an der Tafel ein Fettdruck schlecht möglich ist, werden die Vektoren dort mit
Pfeilen verziert, wir schreiben an der Tafel also $\vec{x}$ an Stelle von $\mathbf{x}$.
\eoxs
\end{Definition}
...
$V$ als $\mathbb{K}$-Vektor-Raum und meinen damit, das das Tripel $\bigl\langle \langle V, \mathbf{0}, +
...
\exercise
Beweisen Sie, dass $\mathbb{K}^n$ ein $\mathbb{K}$-Vektor-Raum ist. \eox


Der Begriff des Vektor-Raums versucht, die wesentliche algebraische Struktur der Menge
$\mathbb{K}^n$ axiomatisch zu erfassen.  Das ist deswegen nützlich, weil es neben dem Vektor-Raum
$\mathbb{K}^n$ noch viele andere Beispiele für Räume 
gibt, welche die selbe algebraische Struktur wie der Raum $\mathbb{K}^n$ aufweisen. 


\example
Es sei $\mathcal{C}(\mathbb{R})$ die Menge der Funktionen $f: \mathbb{R} \rightarrow \mathbb{R}$, die stetig sind, also
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{C}(\mathbb{R}) := \bigl\{ f \in \mathbb{R}^\mathbb{R} \mid \mbox{$f$ ist stetig} \bigr\}$.
\\[0.2cm]
Definieren wir die Addition zweier Funktionen punktweise, definieren wir für $f,g \in \mathcal{C}(\mathbb{R})$ also die
Funktion $f+g$ indem wir
\\[0.2cm]
\hspace*{1.3cm}
$(f+g)(x) := f(x) + g(x)$  \quad f.a. $x \in \mathbb{R}$
\\[0.2cm]
setzen, so ist die so definierte Funktion $f + g$ wieder stetig.  Für ein $\alpha \in \mathbb{R}$ und $f \in
\mathcal{C}(R)$ definieren wir die Funktion $\alpha \cdot f$ als
\\[0.2cm]
\hspace*{1.3cm}
$(\alpha \cdot f)(x) := \alpha \cdot f(x)$ \quad f.a. $x \in \mathbb{R}$.
\\[0.2cm]
Dann ist auch die Funktion $\alpha \cdot f$ stetig.  Schließlich definieren wir eine Funktion $\mathbf{0}:
\mathbb{R} \rightarrow \mathbb{R}$, indem wir
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{0}(x) := 0$ \quad f.a. $x \in \mathbb{R}$ 
\\[0.2cm]
setzen.  Offensichtlich ist diese Funktion stetig und daher gilt $\mathbf{0} \in \mathcal{C}(\mathbb{R})$.  Nun können Sie leicht nachprüfen, dass die Struktur
\\[0.2cm]
\hspace*{1.3cm}
$\bigl\langle \langle \mathcal{C}, \mathbf{0}, + \rangle, \cdot \bigr\rangle$
\\[0.2cm]
ein Vektor-Raum ist.  Das folgt einfach daraus, dass das Assoziativ-Gesetz und das Distributiv-Gesetz für reelle Zahlen
gilt.
 \eoxs
...
\\[0.2cm]
so wird die Menge $\mathbb{K}^\mathbb{N}$ zu einem Vektor-Raum.
\eox

Gegenstand der nächsten Aufgabe ist der Nachweis, dass die Skalar-Multiplikation mit $0$, $1$ und
$-1$ sich so verhält, wie wir es intuitiv erwarten würden.

\exercise
Es sei $V$ ein $\mathbb{K}$-Vektor-Raum.  Beweisen Sie:
...
ein.  Dazu definieren wir zunächst die Begriffe \emph{Erzeugenden-System} und \emph{lineare Unabhängigkeit}.
...
\emph{triviale Linear-Kombination} aus Vektoren der Menge $B$ darstellen.  Demgegenüber heisst eine
...
\\[0.2cm]
\hspace*{1.3cm}
$\sum\limits_{i=1}^n \alpha_i \cdot \mathbf{x}_i = \mathbf{0}$ \quad aber andererseits \quad
...
Gleichung $\alpha_1 = - \alpha_2$ folgt.  Ersetzen wir nun in der letzten Gleichung sowohl
$\alpha_1$ als auch $\alpha_3$ durch $\alpha_2$, so erhalten wir die neue Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Definition}[Erzeugenden-System]
  ...
  $\mathbf{x} \in \mathbf{V}$ als Linear-Kombination von Vektoren aus $B$ schreiben lässt. 
...
Erzeugenden-Systeme, die \emph{minimal} sind.  Dies führt zu der folgenden zentralen Definition.
\pagebreak
...
\exercise
Überlegen Sie, an welcher Stelle die Vorraussetzung $\mathbf{x} \not\in B$ in dem obigen Beweis benutzt wird!
\eox
...
\item $\mathbf{x} \not\in B$.
      ...
      Nach Vorraussetzung ist die Menge $B \cup \{ \mathbf{x} \}$ linear abhängig.  Damit lässt sich
      ...
      \item Der Vektor $\mathbf{x}$ wird bei dieser Linear-Kombination gar nicht benötigt.
            ...
            Vektoren der Menge $B$.  Da die Menge $B$ nach Vorraussetzung linear unabhängig ist,
      ...
      \item Der Vektor $\mathbf{x}$ tritt in der nicht-trivialen Linear-Kombination des
            ...
            \cdots, \mathbf{y}_n$ und Skalare $\alpha_1, \cdots, \alpha_n, \alpha_{n+1}$ so dass
            \\[0.2cm]
            \hspace*{1.3cm}
            ...
            Linear-Kombination gar nicht benötigt, was wir ja bereits ausgeschlossen haben.  Damit
            ...
            \\[0.2cm] 
            umstellen.  Dies zeigt, dass $\mathbf{x}$ sich als Linear-Kombination von Vektoren aus
            $B$ schreiben lässt. 
      \end{enumerate}
\end{enumerate}
...
\proof
Wir führen den Beweis indirekt und nehmen an, dass die Menge $B \backslash \{ \mathbf{x} \}$ doch
ein \\
Erzeugenden-System von $V$ wäre.  Dann müsste sich insbesondere auch der Vektor $\mathbf{x}$ als
Linear-Kombination von Vektoren aus  $B \backslash \{ \mathbf{x} \}$ schreiben lassen.  Es gäbe dann
also ein $n \in \mathbb{N}$ sowie Vektoren $\mathbf{y}_1, \cdots, \mathbf{y}_n$ und Skalare
$\alpha_1, \cdots, \alpha_n$, so dass
\\[0.2cm]
\hspace*{1.3cm}
...
\proof
Da die Menge $B$ nach Vorraussetzung bereits ein Erzeugenden-System von $V$ ist, müssen wir
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{x} = \sum\limits_{j=1}^m \beta_j \cdot \mathbf{z}_j$
\\[0.2cm]
...
\begin{Lemma}[Austausch-Lemma]
  Es sei $V$ ein Vektor-Raum, $B$ eine Basis von $V$, $U \subseteq B$, $\mathbf{x} \in V$ und die
  Menge $U \cup \{ \mathbf{x} \}$ sei linear unabhängig.
  Dann gibt es einen Vektor $\mathbf{y} \in B \backslash U$, so dass die Menge
 $B \backslash \{ \mathbf{y} \} \cup \{ \mathbf{x} \}$ wieder eine Basis von $V$ ist.
\end{Lemma}
...
nicht alle Vektoren $\mathbf{y}_i$ Elemente der Menge $U$ sind, denn wir können wir die obige Gleichung zu
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{y} = \bruch{1}{\alpha_k} \cdot \mathbf{x} - \sum\limits_{i=1 \atop i \not= k}^n
...
\begin{Satz}[Basis-Austausch-Satz]
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $B \backslash W \cup U$ ist eine Basis von V \quad und \quad $\textsl{card}(W) = \textsl{card}(U)$.
\end{Satz}

\proof
Wir definieren $n := \textsl{card}(U)$ und führen den Beweis durch Induktion nach $n$.
\begin{enumerate}
\item[I.A.:] $n = 0$. 
             ...
             \\[0.2cm]
             \hspace*{1.3cm}
             $B \backslash W \cup U = B \backslash \{\} \cup \{\} = B$ \quad und \quad
...
\item[I.S.:] $n \mapsto n+1$. 
             ...
             \\[0.2cm]
             und andererseits die Menge $B \backslash W' \cup U'$  eine Basis von $V$ ist.  Wir wenden nun das Austausch-Lemma
             auf die Basis $B \backslash W' \cup U'$, den Vektor $\mathbf{x}$ und die Menge $U'$ an.  Damit finden wir 
             ein $\mathbf{y} \in B \backslash W'$, so dass
             \\[0.2cm]
             \hspace*{1.3cm}
             $\bigl(B \backslash W' \cup U'\bigr)  \backslash \{ \mathbf{y} \} \cup \{ \mathbf{x} \}$
             \\[0.2cm]
             ...
             \\[0.2cm]
             \hspace*{1.3cm}
             $\bigl(B \backslash W' \cup U'\bigr)  \backslash \{ \mathbf{y} \} \cup \{ \mathbf{x} \} = B \backslash W \cup U$.
             \\[0.2cm]
             Außerdem gilt
             \\[0.2cm]
             \hspace*{1.3cm}
...
dass zwei verschiedene Basen eines Vektor-Raums die selbe Anzahl von Elementen haben. 
...
\example
Es sei $\mathcal{C}(\mathbb{R})$ die Menge der (bereits früher definierten) stetigen reellwertigen Funktionen.
Weiter sei $c \in \mathbb{R}$ beliebig.  Definieren wir die Menge $N_c$ als
\\[0.2cm]
\hspace*{1.3cm}
$N_c := \{ f \in \mathcal{C}(\mathbb{R}) \mid f(c) = 0 \}$,
\\[0.2cm]
also als die Menge der stetigen Funktionen $f$, die bei $c$ eine Nullstelle haben, dann ist
$N_c$ ein Untervektor-Raum von $\mathcal{C}(\mathbb{R})$.

\exercise
Beweisen Sie, dass $N_c$ ein Untervektor-Raum von $\mathcal{C}(\mathbb{R})$ ist.
