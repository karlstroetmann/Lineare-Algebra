...
Die Gleichung $x^2 = -1$ hat in den reelen Zahlen keine Lösung.  Wir wollen uns überlegen,
ob es eventuell möglich ist, die Menge der reellen Zahlen so zu erweitern, dass die
...
$i \cdot i = -1$
\\[0.2cm]
gelten.  Wir definieren dann die Menge $\mathbb{C}$ der \emph{komplexen Zahlen} als Menge
von Paaren
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
damit ein Körper wird und das gleichzeitig für $i$ die Gleichung $i \cdot i = -1$ erfüllt
ist.  Zur Vereinfachung der Schreibweise werden wir dass Paar 
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x,y)$ \quad oft auch als \quad $x + i \cdot y$
\\[0.2cm]
schreiben.  Das diese Schreibweise tatsächlich sinnvoll ist, sehen wir später.
...
sowohl das Assoziativ-Gesetz als auch das Kommutativ-Gesetz gilt und das weiterhin das
...
und das dabei $i \cdot i = -1$ gilt.  Wir rechnen ganz unbefangen das Produkt 
...
Auch die Gültigkeit des Assoziativ-Gesetzes läßt sich nachrechnen: Die Rechnung ist zwar etwas länger,
...
potenzieren, denn nach Gleichung \ref{eq:komplex_mult2} gilt für alle natürlichen Zahlen $n \in \mathbb{N}$
\begin{equation}
  \label{eq:komplex_potenz}
  \bigl(\cos(\varphi) + i \cdot \sin(\varphi)\bigr)^n = \cos(n \cdot \varphi) + i \cdot \sin(n \cdot \varphi).
\end{equation}
Diese Formel wird auch als \href{http://de.wikipedia.org/wiki/Abraham_de_Moivre}{\emph{Satz von de Moivre}} bezeichnet.  Sie kann auch zum Ziehen
...
\\[0.2cm]
\hspace*{1.3cm}
$\zeta_n := \cos\bigl(\frac{2 \cdot \pi}{n}\bigr) + i \cdot \sin\bigl(\frac{2 \cdot \pi}{n}\bigr)$
\\[0.2cm]
als die \emph{primitive} $n$-te Einheitswurzel und sehen, dass die Zahlen
\\[0.2cm]
\hspace*{1.3cm}
$\zeta_n^k := \cos\bigl(\frac{2 \cdot k \cdot \pi}{n}\bigr) + i \cdot \sin\bigl(\frac{2 \cdot k \cdot \pi}{n}\bigr)$
...
\\[0.2cm]
\hspace*{1.3cm}
$\zeta_3 = \cos\bigl(\frac{2 \cdot \pi}{3}) + i \cdot \sin(\frac{2 \cdot \pi}{3}\bigr)
...
\\[0.2cm]
\hspace*{1.3cm}
$\zeta_3^2 = \cos\bigl(\frac{4 \cdot \pi}{3}) + i \cdot \sin(\frac{4 \cdot \pi}{3}\bigr)
 = - \bruch{1}{2} - i \cdot \bruch{\sqrt{3\,}}{2}. 
$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle r = \zeta_n^k \cdot \sqrt[n]{r\,} \cdot 
     \biggl(\cos\Bigl(\frac{\varphi}{n}\Bigr) + i \cdot \sin\Bigl(\frac{\varphi}{n}\Bigr) \biggr)
$
\\[0.2cm] 
eine Lösung der Gleichung $r^n = z$.

\example Wir berechnen alle Lösungen der Gleichung $r^3 = 1 + i$.  Dazu müssen wir zuächst die Zahl $1 +
...
\\[0.2cm]
\hspace*{1.3cm} $1 + i = \sqrt{2\,} \cdot \Bigl(\cos\bigl(\frac{\pi}{4}\bigr) + i \cdot
\sin\bigl(\frac{\pi}{4}\bigr)\Bigr)$.
\\[0.2cm]
Damit erhalten wir dann als eine dritte Wurzel der Zahl $1 + i$ den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
$r := \sqrt[6]{2\,} \cdot 
      \Bigl(\cos\bigl(\frac{\pi}{12}\bigr) + i \cdot \sin\bigl(\frac{\pi}{12}\bigr)\Bigr) 
$.
\\[0.2cm]
Berücksichtigen wir noch, dass die Werte der trigonometrischen Funktionen für das Argument
$\frac{\pi}{12}$ bekannt sind, es gilt nämlich
\\[0.2cm]
\hspace*{1.3cm} 
$\cos\bigl(\frac{\pi}{12}\bigr) = \frac{1}{4} \cdot (\sqrt{6} + \sqrt{2})$ \quad und \quad
$\sin\bigl(\frac{\pi}{12}\bigr) = \frac{1}{4} \cdot (\sqrt{6} - \sqrt{2})$,
\\[0.2cm]
...
Funktionen auf $\frac{\varphi}{3} = \frac{\pi}{12}$ geschlossene Terme angeben.  Normalerweise
...
\\[0.2cm]
um.  Addieren wir auf beiden Seiten die quadratische Ergänzung $\frac{q^2}{4}$, so erhalten wir
...
$
\\[0.2cm]
Berücksichtigen wir, dass $\alpha = u^3$, $\beta = v^3$ und $x = u + v$ ist,
...
$2 - i \cdot 11$ läßt sich jeweils ein expliziter Wert angeben, es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Sie sehen, dass wir ein Problem, dass mit komplexen Zahlen eigentlich nichts zu tun hat, durch die
...
$x_{k} = 2 \cdot \sqrt{\frac{p}{3}\;} \cdot 
    ...
    -(k-1) \cdot \frac{2 \cdot \pi}{3}\right)
$ \quad for $k = 1,2,3$.
\\[0.2cm]
...
\begin{enumerate}
\item $e^x = 1 + x + \frac{1}{2} \cdot x^2 + \frac{1}{6} \cdot x^3 + \cdots = \sum\limits_{n=0}^\infty \frac{1}{n!} \cdot x^n$,
\item $\sin(x) = x - \frac{1}{6} \cdot x^3 + \cdots = \sum\limits_{n=0}^\infty \frac{(-1)^{n}}{(2 \cdot n + 1)!} \cdot x^{2 \cdot n + 1}$,
\item $\cos(x) = 1 -  \frac{1}{2} \cdot x^2 +  \frac{1}{24} \cdot x^4 + \cdots = \sum\limits_{n=0}^\infty  \frac{(-1)^{n}}{(2 \cdot n)!} \cdot x^{2 \cdot n}$.
\end{enumerate}
...
\\[0.2cm]
im Körper der komplexen Zahlen eine Nullstelle hat, d.h.~es gibt ein $z_1 \in \mathbb{C}$, so dass
...
Tatsache entnehmen, das der bedeutendste Mathematiker, der je gelebt hat, 
