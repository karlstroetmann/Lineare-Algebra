...
\begin{Definition}[Monoid]
  Ein Tripel $\langle G, \circ \rangle$ ist ein \emph{Monoid} genau dann, wenn Folgendes gilt:
  \begin{enumerate}
  \item $G$ ist eine Menge,
  ...
  \item Es existiert ein neutrales Element, es gibt also ein $e \in G$, so dass
        \\[0.2cm]
        \hspace*{1.3cm}
        $e \circ a = a \circ e = a$ 
        \\[0.2cm]
        für alle $a \in G$ gilt. \eox
  \end{enumerate}
  \end{enumerate}
\end{Definition}
...
\examples
\begin{enumerate}
\item Die Struktur $\langle \mathbb{Z}, \cdot \rangle$ ist ein Monoid, denn für die Multiplikation
...
\item Es sei $\Sigma$ ein Alphabet, also ein Menge von Buchstaben und $\Sigma^*$ sei die Menge aller
      ...
      wir diese Wörter als Strings.  Wir definieren für Strings eine Multiplikation
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      außerdem für die Konkatenation von Strings das Assoziativ-Gesetz gilt, ist $\langle \Sigma^*, \cdot \rangle$ ein
      Monoid. 
      \eox
\end{enumerate}

\exercise
Zeigen Sie, dass in einem Monoiden $\langle G, \circ \rangle$ das neutrale Element eindeutig
bestimmt ist.  
\eox
