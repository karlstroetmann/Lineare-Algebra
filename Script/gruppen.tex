...
\examples
Bevor wir Sätze über Gruppen beweisen, präsentieren wir zunächst einige Beispiele, an Hand
...
immer nur noch von einem inversen Element und lassen den Zusatz ``\emph{links}'' bzw.~``\emph{rechts}'' weg.
...
immer nur noch von einem neutralen Element und lassen den Zusatz ``\emph{links}'' bzw.~``\emph{rechts}'' weg.
...
Interessant ist nun, dass sich dies auch umkehren läßt, denn es gilt der folgende Satz.
...
\begin{Satz}
  Es sei $M$ eine endliche Menge und die Funktion 
\\[0.2cm]
\hspace*{1.3cm}
$f:M \rightarrow M$
\\[0.2cm]
...
\proof
Da $f$ injektiv ist, werden verschiedene Argumente auch auf verschiedene Werte abgebildet.
Damit hat die Funktion $f$ genau so viele Werte, wie sie Argumente hat, es gilt
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{card}\bigl(f(M)\bigr) = \textsl{card}(M)$.
\\[0.2cm]
Hierbei steht $f(M)$ für das Bild der Menge $M$, es gilt also
\\[0.2cm]
\hspace*{1.3cm}
$f(M) = \bigl\{ f(x) \mid x \in M \bigr\}$.
\\[0.2cm]
Nun gilt aber $f(M) \subseteq M$ und wenn die Mengen $M$ und $f(M)$ die gleiche Anzahl Elemente haben, 
dann kann das bei einer endlichen Menge nur dann gehen, wenn 
\\[0.2cm]
\hspace*{1.3cm}
$f(M) = M$ 
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\bigl\{ f(x) \mid x \in M \bigr\} = M$.
\\[0.2cm]
Damit gibt es also für jedes $y \in M$ ein $x \in M$, so dass $y = f(x)$ gilt und folglich ist $f$
surjektiv.
\qed
...
\\[0.2cm]
Lösungen haben, denn dann folgt die Behauptung aus Satz \ref{satz:solvable}.  Zunächst definieren wir für
...
hat.  Genauso läßt sich  zeigen, dass für beliebige $a,b \in G$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
$y \circ a = b$
\\[0.2cm]
...
Menge $\mathbb{Z}_n^+$ endlich ist, ist die Funktion $R$ damit auch injektiv, denn wenn es ein $x_1,x_2,y \in \mathbb{Z}_n^+$
gäbe, so dass
\\[0.2cm]
\hspace*{1.3cm} 
$\pair(x_1,y) \in R$, \quad
$\pair(x_2,y) \in R$, \quad und \quad
$x_1 \not= x_2$
\\[0.2cm]
gelten würde, dann könnte $R$ nicht mehr surjektiv sein.  Wir definieren nun $\mathcal{S}_n$ als die Menge aller
...
\\[0.2cm]
\hspace*{1.3cm}
$E_n := \{ \pair(x,x) \mid x \in \mathbb{Z}_n^+ \}$.
\\[0.2cm]
...
haben ebenfalls gesehen, dass für die identische Permutation $E_n$ die Beziehung
\\[0.2cm]
\hspace*{1.3cm}
$E_n \circ R = R$ \quad für alle $R \in \mathcal{S}_n$
\\[0.2cm]
...
Struktur $\langle \mathcal{S}_n, E_n, \circ \rangle$ eine Gruppe ist.
...
\item In der Gruppe $\langle \mathbb{Z}, 0, + \rangle$ ist die Menge
      ...
      \end{enumerate}
\item Das letzte Beispiel läßt sich verallgemeinern: Ist $k \in \mathbb{N}$ und definieren wir
      \\[0.2cm]
      \hspace*{1.3cm}
      $k \mathbb{Z} := \{ k \cdot x \mid x \in \mathbb{Z} \}$
      \\[0.2cm]
      als die Menge der Vielfachen von $k$, so läßt sich genau wie in dem letzten Beispiel zeigen, dass
...
\item Wir definieren die Menge $G$ als
      ...
      beschriftet ist, finden.  Beispielsweise gilt $a \circ b = c$.  Es läßt sich zeigen, dass
...
\exercise
Berechnen Sie alle Untergruppen der Gruppe $\langle \mathcal{S}_3, E_3, \circ \rangle$.
\exend
\vspace*{0.3cm}
...
\begin{Satz}
 Es sei $\langle G, 0, + \rangle$ eine kommutative Gruppe und $U \leq G$.
 Dann ist $\langle G/U, 0 + U, + \rangle$ mit der oben definierten Addition von Nebenklassen eine Gruppe.
\end{Satz}
...
Zeigen Sie, dass dann $\mathtt{card}(U)$ ein Teiler von $\mathtt{card}(G)$ ist. \exend
\vspace*{0.2cm}

\noindent
\textbf{Hinweis}: Zeigen Sie, dass alle Nebenklassen von $G$ bezüglich der Untergruppe $U$ die selbe
Kardinalität haben.
