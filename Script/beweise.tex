...
sind algebraische Umformungen und Fallunterscheidungen.  Wir geben ein einfaches Beispiel für einen
direkten Beweis, benötigen dafür aber zunächst noch eine Definition.


\begin{Definition}[Pythagoräische Tripel] \hspace*{\fill} \\
Ein Tripel $\langle x, y, z \rangle \in \mathbb{N}\!\times\!\mathbb{N}\!\times\!\mathbb{N}$ heißt
\emph{Pythagoräisches Tripel}, falls
\\[0.2cm]
\hspace*{1.3cm}
$x^2 + y^2 = z^2$
\\[0.2cm]
...
\example
Das Tripel $\langle 3, 4, 5 \rangle$ ist ein pythagoräisches Tripel, denn es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Satz} 
Es seien $u$ und $v$ positive natürliche Zahlen mit $u > v$.  Dann ist
\\[0.2cm]
\hspace*{1.3cm}
...
Wir geben einige Beispiele für indirekte Beweise.
\vspace*{0.2cm}
...
solche Zahl läßt sich also immer in der Form $n = 2 \cdot k$ mit $k \in \mathbb{N}$ schreiben.
Eine natürliche Zahl $n$  ist ungerade, wenn sie nicht durch $2$ teilbar ist.  Eine ungerade Zahl
hat bei Division durch $2$ also den Rest $1$ und läßt sich damit immer in der Form $2 \cdot k + 1$
...
\proof
Wir nehmen an, dass $p$ ungerade ist.  Damit läßt sich $p$  in der Form
\\[0.2cm]
\hspace*{1.3cm}
...
rationale Zahl läßt sich in der Form $\bruch{p}{q}$ mit $p,q \in \mathbb{Q}$ schreiben.  Dabei können
...
darstellen läßt, denn einen solchen Bruch können wir immer so kürzen, dass $p$ und $q$ teilerfremd
sind. 
\qed


Ein anderes typisches Beispiel für einen indirekten Beweis ist der Nachweis der
Nicht-Abzählbarkeit der Menge Potenz-Menge der natürlichen Zahlen.

\begin{Definition}[Abzählbar] \hspace*{\fill} \\
Eine unendliche Menge $M$ heißt \emph{abzählbar unendlich}, wenn eine 
...
aufgezählt werden, wobei wir eventuelle Wiederholung eines Elements zulassen wollen.
...
Summenformeln sollten Sie sich gut merken, denn diese Formeln werden bei der Analyse der Komplexität
...
\exercise
Es sei $x \in \mathbb{R}$.  Zeigen Sie, dass für alle positiven natürlichen Zahlen die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
...
\exercise 
Zeigen Sie, dass die Zahl $2^{2 \cdot n} - 1$ für alle positiven natürlichen Zahlen $n$ durch $3$
teilbar ist,
\exend 
\pagebreak
