...
\\[0.2cm]
Folglich ist $\vec{x}$ eine Eigenvektor von $A$ zum Eigenwert $3$, während $\vec{y}$ ein
...
\\[0.2cm]
denn wenn wir die Gleichung $\vec{x}_{n+1} = A \cdot \vec{x}_n$ komponentenweise schreiben,
...
Rekurrenz-Gleichung für die Fibonacci-Zahlen.  Aus der Gleichung $\vec{x}_{n+1} = A \cdot\vec{x}_n$ 
...
$
\\[0.2cm]
Die erste Komponente von $\vec{x}_n$ ist gleich $f_n$.  Daher haben wir für die $n$-te
