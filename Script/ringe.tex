...
darstellen läßt.  Dann läßt sich nämlich aus einem Integritäts-Ring, der bezüglich der
Multiplikation ein 
...
erzeugen läßt.  Die Konstruktion ist dabei analog zur Konstruktion der
...
$\mathcal{R} = \langle R, 0, e, +, \cdot \rangle$, wir geben die Eins also explizit in der Struktur an.
\end{Definition}
...
\item Bezeichnen wir das bezüglich der Addition $+$ zu einem Element $x$ inverse Element mit $-x$,
      ...
      \\[0.2cm]
      Wir zeigen die erste dieser beiden Gleichungen, die zweite läßt sich analog nachweisen.
...
als die \emph{Multiplikation} bezeichnen.
\end{Definition}
...
\\[0.2cm]
Es läßt sich zeigen, dass die so definierte Struktur $\mathcal{R}$ ein Ring ist.  In
diesem Ring gilt
\\[0.2cm]
\hspace*{1.3cm}
...
$2 = 0$
\\[0.2cm]
hergeleitet.  Dieser Wiederspruch zeigt, dass sich der Ring $\mathcal{R}$ sicher nicht zu
einem Körper erweitern läßt.
...
\begin{Definition}[Integritäts-Ring]
  ...
  \\[0.2cm]
  gilt, die Zahl $0$ läßt sich in einem nullteilerfreien Ring also nur als triviales Produkt darstellen.
  Ein nullteilerfreier, kommutativer Ring, der eine Eins hat, wird als \emph{Integritäts-Ring} bezeichnet.
\end{Definition}
...
so läßt sich auf dem Quotienten-Raum $R/\!\sim$ unter bestimmten Umständen ebenfalls eine Ring-Struktur
...
\\[0.2cm]
\hspace*{1.3cm}
$[a]_R = [b]_R \;\leftrightarrow\; a \mathop{R} b$
\\[0.2cm]
...
$[a_1 + b_1]_\sim = [a_2 + b_2]_\sim$. 
\\[0.2cm]
Genauso läßt sich zeigen, dass auch die Multiplikation $\cdot$ auf $R/\!\sim$ wohldefiniert ist.
...
Dann können wir durch eine Funktion 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{char}(\mathcal{K}) = \min\bigl(\{ k \in \mathbb{N} \mid k \geq 1 \wedge \mathtt{count}(k) = 0_K \}\bigr)$.
\\[0.2cm]
...
\begin{enumerate}
\item $\langle I, 0, + \rangle \leq \langle R, 0, + \rangle$,
...
\item $\forall a \in I: \forall b \in R: b \cdot a \in I$,
      ...
      $b$ aus dem Ring $R$ wieder ein Element aus dem Ideal.
\end{enumerate}

\remark
An dieser Stelle sollten Sie sich noch einmal die Definition einer Untergruppe ins
Gedächtnis rufen:  Es gilt $\langle I, 0, + \rangle \leq \langle R, 0, + \rangle$ genau
...
\item Die Menge alle geraden Zahlen
      ...
      \end{enumerate}
\item Das letzte Beispiel läßt sich verallgemeinern: Es sei $k \in \mathbb{Z}$. Dann ist die Menge 
      \\[0.2cm]
      \hspace*{1.3cm}
...
Mit Hilfe von Idealen läßt sich auf einem Ring eine Kongruenz-Relation erzeugen.  Ist $I$ ein Ideal auf
