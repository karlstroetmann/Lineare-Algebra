...
Um Determinanten einführen zu können, müssen wir zunächst das \emph{Signum} einer Permutation definieren.
...
Zukunft der Kürze halber die Permutation $R$ auch als die Liste 
\\[0.2cm]
\hspace*{1.3cm}
$R = [x_1,\cdots,x_n]$
\\[0.2cm]
...
Transpositionen sind einfacher als beliebige Permutationen und wir werden später insbesondere sehen, dass sich
die noch zu definierende Funktion $\mathtt{sgn}(R)$, die das \emph{Vorzeichen} einer
Permutation berechnet, für Transpositionen sofort berechnen lässt.  Da der nächste Satz uns zeigt, dass
...
\begin{Satz} \label{satz:produkt-von-transpositionen}
  Ist $n \geq 2$ und gilt $R \in \mathcal{S}_n$, so lässt sich $R$ als Produkt von Transpositionen darstellen.
\end{Satz}
...
\item[I.S.:] $n \mapsto n + 1$.
             ...
             \item Fall: $k = n+1$.
                   ...
                   In diesem Fall enthält die Permutation $R$ also das Paar $\pair(n+1,n+1)$.  Dann
                   definieren wir
                   \\[0.2cm]
                   \hspace*{1.3cm}
                   $R' := R \backslash \{ \pair(n+1,n+1) \}$.
                   \\[0.2cm]
                   Damit ist $R' \in \mathcal{S}_n$ und lässt sich folglich nach Induktions-Voraussetzung als
             ...
             \item Fall:  $k \leq n$.                   
                   ...
                   \\[0.2cm]
                   Damit bildet $R'$ also die Zahl $n+1$ auf sich selbst ab und folglich erfüllt
                   $R'$ die Voraussetzung des ersten Falls.  Da wir die Gültigkeit unserer Behauptung für
...
\begin{Definition}[Signum]
  Für Permutationen $R \in \mathcal{S}_n$ definieren wir für alle positiven natürlichen Zahlen $n$ eine Funktion
  \\[0.2cm]
  \hspace*{1.3cm}
  $\texttt{inv} : \mathcal{S}_n \mapsto 2^{\mathbb{Z}_n^+ \times \mathbb{Z}_n^+}$
  \\[0.2cm]
  die jeder Permutation $R \in \mathcal{S}_n$ eine Menge von Paaren natürlicher Zahlen $k$ und $l$
  zuordnet, für die
  \\[0.2cm]
  \hspace*{1.3cm}
...
\begin{Lemma}
  ...
  $x_k$ und $x_{k+1}$ in der Liste, die $R$ repräsentiert, ihre Positionen tauschen.
\end{Lemma}

\proof
Den Beweis der obigen Gleichung für $\tau \circ R$ können
...
der Beweis ist aber andererseits elementarer und daher für den mathematisch weniger geübten
...
\\[0.2cm]
Damit schreibt sich die Menge $\mathtt{inv}(R)$ kürzer als
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{inv}(R) = \bigl\{ \pair(i,j) \in P \mid R(i) > R(j) \bigr\}$.
\\[0.2cm]
Wir müssen untersuchen, wie sich die Mengen $\mathtt{inv}(R)$ und $\mathtt{inv}(\tau \circ R)$
...
\end{enumerate} 
Entsprechend der Zerlegung von $P$ können wir nun auch die Menge $\mathtt{inv}(R)$ in sechs
paarweise disjunkte Teile zerlegen.  Wir definieren
\\[0.2cm]
\hspace*{1.3cm}
$A_s := \bigl\{ \pair(i,j) \in P_s \mid R(i) > R(j) \bigr\}$ \quad für $s \in \{1,\cdots,6\}$.
\\[0.2cm]
Damit gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$B_s := \bigl\{ \pair(i,j) \in P_s \mid R(\tau(i)) > R(\tau(j)) \bigr\}$ \quad für $s \in \{1,\cdots,6\}$.
\\[0.2cm]
Damit gilt offenbar
\\[0.2cm]
\hspace*{1.3cm}
...
\item Falls $\pair(i,j) \in P_1$ ist, sind $i$ und $j$ sowohl von $k$ als auch von $k+1$
      ...
      \\[0.2cm]
      Folglich ist $A_1 = B_1$ und daraus folgt, dass die Mengen $A_1$ und $B_1$ die selbe
      Kardinalität haben:
      \\[0.2cm]
      \hspace*{1.3cm}
...
Da sich jede Permutation $R \in \mathcal{S}_n$ als Produkt von Transpositionen darstellen lässt,
sehen wir nun, dass das Vorzeichen $\mathtt{sgn}(R)$ genau dann den Wert $+1$ hat, wenn $R$ sich
als Produkt einer geraden Anzahl von Permutationen darstellen lässt.  Andernfalls hat $\mathtt{sgn}(R)$
...
\href{http://de.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz}{Gottfried Wilhelm Leibniz\footnote{
 ...
 die Firma \href{http://www.bahlsen.com}{Bahlsen} den Butterkeks tatsächlich nach dem Mathematiker
...
$
\\[0.2cm]
angeben.  Dabei bezeichnet $\mathbf{a}_i$ den $i$-ten Zeilenvektor der Matrix $A = (a_{i,j})$, es
gilt also
\\[0.2cm]
\hspace*{1.3cm}
...
$\mathtt{det}\left(
  \begin{array}{ll}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a}_l \\ \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right) = - \mathtt{det}\left(
  \begin{array}{ll}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a_k} \\ \vdots \\ \mathbf{a}_l \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
$.
\\[0.2cm]
...
$\left(
  \begin{array}{ll}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a_l} \\ \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
$
\\[0.2cm]
...
wir die Eigenschaften der Funktion $\mathtt{sgn}$ tatsächlich benötigen.  Der Rest der Theorie läuft
...
\exercise
Zeigen Sie, dass für die $n \times n$ Einheits-Matrix $E_n = (\delta_{i,j})_{{i =1,\cdots n} \atop {j=1,\cdots,n}}$
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{det}(E_n) = 1$
\\[0.2cm]
...
$\mathtt{det}\left(
  \begin{array}{ll}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_n
  \end{array} \right)
$
\\[0.2cm]
...
\mathtt{det}\left(
  \begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{b} + \mathbf{c}\\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
 = \mathtt{det}\left(
  \begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{b} \\ \vdots \\ \mathbf{a}_n
  \end{array}  \right)
+
   \mathtt{det}\left(
  \begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{c} \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
$.
\eoxs
...
\mathtt{det}\left(
  \begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \lambda \cdot \mathbf{b} \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
...
\item Alternierend ist die Determinante, weil
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{det}(\cdots, \mathbf{ a}_ k, \cdots, \mathbf{ a}_l, \cdots) = -\mathtt{det}(\cdots, \mathbf{ a}_l, \cdots, \mathbf{ a}_k, \cdots)$
      \\[0.2cm]
      gilt.  Das Vertauschen zweier Spalten ändert das Vorzeichen der Determinante.
\item Multilinear ist die Determinante, weil
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{det}(\cdots, \alpha \cdot \mathbf{a} + \beta \cdot \mathbf{b}, \cdots) = 
         \alpha \cdot \mathtt{det}(\cdots, \mathbf{a}, \cdots) +
         \beta  \cdot \mathtt{det}(\cdots, \mathbf{b}, \cdots)
      $
      \\[0.2cm]
...
\begin{Lemma} \label{lemma:det-add-line}
  ...
  $A = \left(
  \begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_l \\ \vdots \\ \mathbf{a}_n
  \end{array} \right)
  $ \quad
  hat, gilt also \quad
  $B = 
  \left(\begin{array}{c}
    \mathbf{a}_1 \\ \vdots \\ \mathbf{a_k} \\ \vdots \\ \mathbf{a}_l + \lambda \cdot \mathbf{a}_k \\ \vdots \\ \mathbf{a}_n
  \end{array} 
  \right)
$.
  \\[0.2cm]
...
\hspace*{1.3cm}
  $C := \left(
  \begin{array}{c}
    \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_k \\ \vdots 
  \end{array} \right)
  $,
\\[0.2cm]
...
$\mathtt{det}(D) = \mathtt{det}\left(
  \begin{array}{c}
    \vdots \\ \mathbf{a}_k \\ \vdots \\ \lambda \cdot \mathbf{a}_k \\ \vdots 
  \end{array} \right) =
  \lambda \cdot \mathtt{det}\left(
  \begin{array}{c}
    \vdots \\ \mathbf{a}_k \\ \vdots \\ \mathbf{a}_k \\ \vdots 
...
\mathtt{det}(B) & = & 
  \mathtt{det}\left(\begin{array}{c}
    \vdots \\ \mathbf{a_k} \\ \vdots \\ \mathbf{a}_l + \lambda \cdot \mathbf{a}_k \\ \vdots 
  \end{array} 
  \right) \\[1.5cm]
 & = &
\mathtt{det}\left(\begin{array}{c}
  \vdots \\ \mathbf{a_k} \\ \vdots \\ \mathbf{a}_l \\ \vdots 
  \end{array} 
  \right) +
  \mathtt{det}\left(\begin{array}{c}
    \vdots \\ \mathbf{a_k} \\ \vdots \\ \lambda \cdot \mathbf{a}_k \\ \vdots 
  \end{array} 
  \right) \\[1.5cm]
...
\begin{Lemma}
  Ist $A = (\mathbf{a}_1, \cdots, \mathbf{a}_n) \in \mathbb{K}^{n \times n}$ und ist die Menge der
  Spalten der Matrix $A$, also die Menge $\{ \mathbf{a}_1, \cdots, \mathbf{a}_n \}$, linear abhängig,
  so gilt
  \\[0.2cm]
  \hspace*{1.3cm}
  $\mathtt{det}(A) = 0$.
...
der Spalten von $B$ der Null-Vektor $\mathbf{0}$ ist.  Dies können wir wie folgt aus der Homogenität
der Determinanten-Funktion schließen: Nehmen wir an, dass die $i$-te Spalte von $B$ der Null-Vektor
ist, es gelte also
\\[0.2cm]
\hspace*{1.3cm}
$B = (\mathbf{b}_1,\cdots, \mathbf{b}_{i-1}, \mathbf{0}, \mathbf{b}_{i+1}, \cdots, \mathbf{b}_n)$.
\\[0.2cm]
Dann haben wir 
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
 \mathtt{det}(B) & = & \mathtt{det}(\mathbf{b}_1,\cdots, \mathbf{b}_{i-1}, \mathbf{0}, \mathbf{b}_{i+1}, \cdots, \mathbf{b}_n) \\[0.2cm] 
                 & = & \mathtt{det}(\mathbf{b}_1,\cdots, \mathbf{b}_{i-1}, 0 \cdot \mathbf{0}, \mathbf{b}_{i+1}, \cdots, \mathbf{b}_n) \\[0.2cm]  
                 & = & 0 \cdot \mathtt{det}(\mathbf{b}_1,\cdots, \mathbf{b}_{i-1}, \mathbf{0}, \mathbf{b}_{i+1}, \cdots, \mathbf{b}_n) \\[0.2cm]  
                 & = & 0.
\end{array}
$
\\[0.2cm]
Aus der Vorraussetzung, dass die Menge $\{\mathbf{a}_1, \cdots, \mathbf{a}_n\}$ linear abhängig ist, folgt,
dass es Skalare $\lambda_1,\cdots, \lambda_n \in \mathbb{K}$ gibt, so dass 
\\[0.2cm]
\hspace*{1.3cm}
$\lambda_1 \cdot \mathbf{a}_1 + \cdots + \lambda_n \cdot \mathbf{a}_n = \mathbf{0}$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{a}_k = - \sum\limits_{i=1 \atop i \not= k}^n \bruch{\lambda_i}{\lambda_k} \cdot \mathbf{a}_i$.
\\[0.2cm]
...
$
\begin{array}[t]{lcl}
  \mathtt{det}(A) & = & \mathtt{det}(\mathbf{a}_1, \cdots, \mathbf{a}_k, \cdots, \mathbf{a}_n) \\[0.2cm]
                  & = & \mathtt{det}(\mathbf{a}_1, \cdots, 
                        \mathbf{a}_k - \bruch{\lambda_1}{\lambda_k} \cdot \mathbf{a_1}, \cdots, \mathbf{a}_n) \\[0.2cm]
                  & = & \mathtt{det}(\mathbf{a}_1, \cdots, 
                        \mathbf{a}_k -\bruch{\lambda_1}{\lambda_k} \cdot \mathbf{a_1} - \bruch{\lambda_2}{\lambda_k} \cdot \mathbf{a_2},
                        \cdots, \mathbf{a}_n) \\[0.2cm] 
                  & = & \vdots \\
                  & = & \mathtt{det}(\mathbf{a}_1, \cdots, 
                        \mathbf{a}_k - \sum\limits_{i=1 \atop i \not= k}^n \bruch{\lambda_i}{\lambda_k} \cdot \mathbf{a}_i,
                        \cdots, \mathbf{a}_n) \\[0.2cm] 
...
$
\\[0.2cm]
Im letzten Schritt haben wir dann eine Matrix, deren $k$-te Spalte den Wert $\mathbf{0}$ hat und
...
\begin{enumerate}
\item $\mathtt{Kern}(A) := \bigl\{ \mathbf{x} \in \mathbb{K}^m \bigm| A \cdot \mathbf{x} = \mathbf{0} \bigr\}$, 
\item $\mathtt{Bild}(A) := \bigl\{ A \cdot \mathbf{x} \bigm| \mathbf{x} \in  \mathbb{K}^m \bigr\}$.
\end{enumerate}
Da die Abbildung $\mathbf{x} \mapsto A \cdot \mathbf{x}$ eine lineare Abbildung ist, ist $\mathtt{Kern}(A)$ ein Unter-Vektorraum 
...
impliziert, dass die Abbildung $\mathbf{x} \mapsto A \cdot \mathbf{x}$ injektiv ist, während
die Bedingung $\mathtt{dim}(\mathtt{Bild}(A)) = n$ impliziert, dass die Abbildung $\mathbf{x} \mapsto A \cdot \mathbf{x}$
surjektiv ist.  Folglich ist diese Abbildung insgesamt bijektiv und besitzt damit eine Umkehr-Abbildung.
Bezeichnen wir die Abbildung $\mathbf{x} \mapsto A  \cdot x$ mit $f$, setzen wir also
\\[0.2cm]
\hspace*{1.3cm}
...
$
\begin{array}[t]{ll}
                & f^{-1}(\mathbf{x} + \mathbf{y}) = f^{-1}(\mathbf{x}) + f^{-1}(\mathbf{y}) \\[0.2cm]
\Leftrightarrow & f\bigl(f^{-1}(\mathbf{x} + \mathbf{y})\bigr) = f\bigl(f^{-1}(\mathbf{x}) + f^{-1}(\mathbf{y})\bigr) \\
                & \mbox{denn $f$ ist injektiv} \\[0.2cm] 
\Leftrightarrow & \mathbf{x} + \mathbf{y} = f\bigl(f^{-1}(\mathbf{x})) + f\bigl(f^{-1}(\mathbf{y})\bigr) \\
                & \mbox{denn $f$ ist linear} \\[0.2cm] 
\Leftrightarrow & \mathbf{x} + \mathbf{y} = \mathbf{x} + \mathbf{y}.
\end{array}
$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$f^{-1}\bigl(f(\mathbf{x})\bigr) = \mathbf{x}$ \quad und \quad
$f\bigl(f^{-1}(\mathbf{x})\bigr) = \mathbf{x}$ 
\\[0.2cm]
dann in der Form 
\\[0.2cm]
\hspace*{1.3cm}
$B \cdot A \cdot \mathbf{x} = \mathbf{x}$ \quad und \quad
$A \cdot B \cdot \mathbf{x} = \mathbf{x}$ 
\\[0.2cm]
...
\begin{Lemma}
  Die Matrix $A \in \mathbf{K}^{n \times n}$ sei invertierbar.  Dann gilt $\mathtt{det}(A) \not= 0$.
\end{Lemma}
...
\item[``$\Leftarrow$'']
     ...
     Nun sei $A$ nicht invertierbar.  Dann gilt $\mathtt{Kern}(A) \not= \{\mathbf{0}\}$.  Also gibt
     es einen Vektor $\mathbf{x}$, so dass $A \cdot \mathbf{x} = \mathbf{0}$ ist.  Schreiben wir
     $A$ als
     \\[0.2cm]
     \hspace*{1.3cm}
     $A = (\mathbf{a}_1, \cdots, \mathbf{a}_n)$,
     \\[0.2cm]
     wobei $\mathbf{a}_1,\cdots,\mathbf{a}_n$ die der Spalten-Vektoren von $A$ sind, so folgt aus
     der Gleichung $A \cdot \mathbf{x} = \mathbf{0}$ die Gleichung
     \\[0.2cm]
     \hspace*{1.3cm}
     $\sum\limits_{i=1}^n x_i \cdot \mathbf{a}_i = \mathbf{0}$,
     \\[0.2cm]
     wobei hier die $x_i$ die Komponenten des Vektors $\mathbf{x}$ bezeichnen.  Damit sind aber die
...
auf das Vorzeichen  die selbe Determinante wie die Matrix $A$ und die Determinante von $D$ lässt
...
\item Die Matrix \texttt{a}, deren Determinante berechnet werden soll, wird als Liste ihrer Zeilen
      dargestellt.  Das ist die selbe Darstellung, die wir auch bei dem in Abbildung
