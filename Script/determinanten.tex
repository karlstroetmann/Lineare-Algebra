...
$\langle \mathcal{S}_n, E_n, \circ \rangle$ auf die Gruppe $\langle \{-1,+1\}, 1, \cdot \rangle$.  Die Definition der Funktion $\mathtt{sgn}$ selbst
...
\exercise
Es sei $E_n \in \mathcal{S}_n$ die identische Permutation aus $R_n$.  Stellen wir diese Permutation
...
Transpositionen sind einfacher als beliebige Permutationen und wir werden gleich sehen, dass sich
für eine Transpositionen $\tau$ das Vorzeichen $\mathtt{sgn}(\tau)$ sofort berechnen lässt, denn es gilt
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{sgn}\bigl(\pair(k,l)\bigr) = -1$.
\\[0.2cm]
Der Nachweis dieser Behauptung ist Gegenstand der folgenden Aufgabe.

\exercise
Die Transposition $\tau$ habe die Form \quad $\tau = \pair(k,l)$ \quad
und es gelte $k < l$.  Berechnen Sie $\mathtt{inv}(\tau)$ und $\mathtt{sgn}(\tau)$.
\eox
...
\item[I.A.:] $k=1$. Es gilt
             \\[0.2cm]
             \hspace*{1.3cm}
             $\mathtt{sgn}(\tau) = \mathtt{sgn}(\tau \circ E_n) = -\mathtt{sgn}(E_n) = -1 = (-1)^1$.
...
\href{http://de.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz}{Gottfried Wilhelm Leibniz\footnote{
  Der erfreuliche Umstand, dass der Name ``Leibniz'' selbst in bildungsfernen Schichten ein fester
