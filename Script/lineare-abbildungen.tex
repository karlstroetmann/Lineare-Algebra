...
indem wir fordern, dass für alle $\vec{x} \in V$
\\[0.2cm]
\hspace*{1.3cm}
$(f+g)(\vec{x}) := f(\vec{x}) + g(\vec{x})$ 
\\[0.2cm]
gilt.  Weiter definieren wir die Null-Abbildung $\vec{0}: V \rightarrow W$, indem wir für alle
$\vec{x} \in V$
\\[0.2cm]
\hspace*{1.3cm}
$\vec{0}(\vec{x}) := \vec{0}$
...
\\[0.2cm]
und wir definieren für alle $\alpha \in \mathbb{K}$, $f \in \mathcal{L}(V, W)$ und alle $\vec{x} \in V$
\\[0.2cm]
\hspace*{1.3cm}
...
\item $\bigl\langle \langle \mathcal{L}(V, W), \vec{0}, + \rangle, \cdot \rangle$ ist ein $\mathbb{K}$-Vektor-Raum.
      \eoxs
\end{enumerate}
...
\begin{Definition}[Kern]
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $\textsl{Kern}(f) := \{ \vec{x} \in V \mid f(\vec{x}) = \vec{0} \}$.
  \\[0.2cm] 
...
\item Wir haben oben bereits gesehen, dass für eine lineare Abbildung
      $f(\vec{0}) = \vec{0}$ gilt.  Daraus folgt sofort $\vec{0} \in \textsl{Kern}(f)$.
...
\begin{Satz}
  ...
  Dann ist $f$ genau dann injektiv, wenn $\textsl{Kern}(f) = \{ \vec{0} \}$ gilt.
\end{Satz}
...
\item[``$\Rightarrow$'':] Wir nehmen zunächst an, dass $f$ injektiv ist.  Es ist klar, dass
      ...
      $f(\vec{0}) = \vec{0}$
      \\[0.2cm]
      gilt und damit gilt sicher $\vec{0} \in \textsl{Kern}(f)$.  Sei nun zusätzlich $\vec{y} \in \textsl{Kern}(f)$, es
      gelte also 
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(\vec{y}) = \vec{0}$.
      \\[0.2cm]
      Da $f$ injektiv ist, folgt aus $f(\vec{y}) = \vec{0}$ und $f(\vec{0}) = \vec{0}$
      \\[0.2cm]
      \hspace*{1.3cm}
      $\vec{y} = \vec{0}$
      \\[0.2cm]
      und damit ist $\textsl{Kern}(f) = \{ \vec{0} \}$ gezeigt.
\item[``$\Leftarrow$'':] Wir setzen jetzt $\textsl{Kern}(f) = \{ \vec{0} \}$ voraus.  Falls für $\vec{x}, \vec{y} \in V$
      nun
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(\vec{x}) = f(\vec{y})$
      ...
      $\vec{x} - \vec{y} \in \textsl{Kern}(f)$
      \\[0.2cm]
      und da wir $\textsl{Kern}(f) = \{ \vec{0} \}$ vorausgesetzt hatten, folgt
      \\[0.2cm]
      \hspace*{1.3cm}
      $\vec{x} - \vec{y} = 0$
      \\[0.2cm]
      woraus wir auf $\vec{x} = \vec{y}$ schließen können und das war zu zeigen.  \qeds
\end{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{Bild}(f) := \{ f(\vec{x}) \mid \vec{x} \in V \}$
\\[0.2cm]
...
\proof
Es sei $\{ \vec{x}_1, \cdots, \vec{x}_m \}$ eine Basis von $\textsl{Kern}(f)$.  Ist $B$ eine Basis von $V_1$, so können wir mit Hilfe des
...
\begin{enumerate}
\item Wir zeigen:
      Die Menge $\{ f(\vec{y}_1), \cdots, f(\vec{y}_n) \}$ ist ein Erzeugenden-System von $\textsl{Bild}(f)$.
      ...
      $f(\vec{x}_i) = \vec{0}$ ist für alle $i \in \{1,\cdots,m\}$, denn $\vec{x}_i \in \textsl{Kern}(f)$, so erhalten wir
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Wir zeigen: Die Menge $\{ f(\vec{y}_1), \cdots, f(\vec{y}_n) \}$ ist linear unabhängig.
...
$\{ \vec{y}_1, \cdots, \vec{y}_n \}$ sei eine Basis von $W$.  Für alle $j \in \{1,\cdots,m\}$ ist zunächst
...
\\[0.2cm]
schreiben lässt.  Ist nun allgemein $\vec{x}$ ein Vektor aus $V$, so lässt sich $\vec{x}$ in der Form 
\\[0.2cm]
\hspace*{1.3cm}
...
Sind $V$ und $W$ zwei $\mathbb{K}$-Vektor-Räume und bilden die Vektoren $\vec{x}_1, \cdots, \vec{x}_m$
...
\remark
Wir hatten die Vektoren aus $\mathbb{K}^n$ ursprünglich als Listen der Form $\vec{x} = [x_1, \cdots, x_n]$ mit
...
\\[0.2cm]
die für alle $\vec{x} \in \mathbb{K}^m$ durch
\\[0.2cm]
\hspace*{1.3cm}
...
Definition ist leicht zu sehen, dass für beliebige Vektoren $\vec{x} \in \mathbb{K}^m$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
dieser Definition gilt für beliebige Vektoren $\vec{x} \in \mathbb{K}^m$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$f(\vec{x}) := A \cdot \vec{x}$ \quad f.a.~$\vec{x} \in \mathbb{K}^l$,
 \quad und \quad
 $g(\vec{y}) := B \cdot \vec{y}$ \quad f.a.~$\vec{y} \in \mathbb{K}^m$,
 \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$h(\vec{x}) := (g \circ f)(\vec{x}) = g(f(\vec{x}))$
\\[0.2cm]
...
\\[0.2cm]
was wegen $A^{-1} \cdot A \cdot \vec{x} = \mathrm{E}_n \cdot \vec{x} = \vec{x}$ zu
\\[0.2cm]
\hspace*{1.3cm}
$\vec{x} = A^{-1} \cdot \vec{b}$
\\[0.2cm]
äquivalent ist.  Falls wir also in der Lage sind, die Matrix $A$ zu invertieren,  dann können wir anschließend
das Lösen der Gleichung $A \cdot \vec{x} = \vec{b}$  auf eine Matrix-Multiplikation zurück
führen.  Da das Inverse $A^{-1}$ nicht von dem Vektor $\vec{b}$ abhängt, ist dies dann ein
sinnvolles Vorgehen, wenn die Gleichung $A \cdot \vec{x} = \vec{b}$ für mehrere verschiedene Werte von
$\vec{b}$ gelöst werden soll.  \eoxs
...
\item Im $i$-ten Durchlauf der Schleife in Zeile 4 wird erreicht, dass die $i$-te Spalte von
      $\mathtt{a}$ mit dem $i$-ten Einheits-Vektor $\vec{e}_i$ übereinstimmt.
      \begin{enumerate}
...
\item In den folgenden Schritten geht es darum, die zweite Spalte der Matrix $\mathtt{a}$ in den
      Einheits-Vektor $\vec{e}_2$ zu überführen.  Zunächst überprüfen wir, für welches $j \geq 2$
...
\item In den verbleibenden Schritten geht es darum, die dritte Spalte der Matrix $\mathtt{a}$ in den
      Einheits-Vektor $\vec{e}_3$ zu überführen.  Wir beginnen damit, dass wir die dritte Zeile
