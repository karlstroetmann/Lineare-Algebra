...
\href{http://en.wikipedia.org/wiki/Field_(mathematics)}{Körper}.  Diese Begriffe werde ich gleich erklären.
...
Zahlen. Ein Ring ist ein \emph{\color{blue}Körper}, wenn zusätzlich auch noch eine Division möglich ist.
...
Die erste Methode funktioniert in sogenannten \emph{\color{blue}Integritäts-Ringen}, das sind
solche Ringe, in denen sich das neutrale Element der Addition, also die $0$, nicht als Produkt
zweier von $0$ verschiedener Elemente darstellen lässt.  Dann lässt sich nämlich aus
einem Integritäts-Ring, der bezüglich der Multiplikation ein 
neutrales Element enthält, ein sogenannter \emph{\color{blue}Quotienten-Körper} erzeugen.
Die Konstruktion dieses Körpers 
verläuft analog zu der Konstruktion der rationalen Zahlen aus den ganzen Zahlen.

Die zweite Methode arbeitet mit Hilfe sogenannter \emph{\color{blue}maximaler Ideale}.  Wir
werden in Ringen zunächst  
\emph{\color{blue}Ideale} definieren.  Dabei sind Ideale das Analogon zu Untergruppen in der Gruppen-Theorie.
Anschließend zeigen wir, wie sich mit Hilfe eines Ideals $I$ auf einem Ring $\mathcal{R}$ eine
\emph{\color{blue}Kongruenz-Relation} 
erzeugen lässt.  Die Konstruktion ist dabei analog zur Konstruktion der
Faktor-Gruppe aus dem letzten Abschnitt.  Für \emph{\color{blue}maximale Ideale} werden wir schließlich zeigen,
...
\item $\cdot: R \times R \rightarrow R$ ist eine Funktion, die wir als die \emph{\color{blue}Multiplikation} in $\mathcal{R}$ bezeichnen.
      Für die Multiplikation m\"ussen die folgenden Gesetze gelten:
      \begin{enumerate}
...
\item Die Multiplikation und die Addition des Rings sind \"uber die folgenden
      ...
      $(x + y) \cdot z = x \cdot z + y \cdot z$ \quad für alle $x, y,z \in R$.
\end{enumerate}
Die Struktur $\mathcal{R}$ ist ein \blue{kommutativer Ring} falls zusätzlich für die Multiplikation
...
$\mathcal{R} = \langle R, 0, 1, +, \cdot \rangle$, für den die Struktur $\langle R \backslash \{0\}, 1, \cdot \rangle$
eine Gruppe ist.  Dieser Spezialfall liegt beispielsweise bei dem Ring  
$\langle \mathbb{Q}, 0, 1, +, \cdot \rangle$ vor.  In einem solchen Fall sprechen wir von einem \blue{Körper}.  Die formale
Definition folgt.


\begin{Definition}[Körper]
Ein $5$-Tupel $\mathcal{K} = \langle K, 0, 1, +, \cdot \rangle$ ist ein \blue{Körper}, falls gilt:
\begin{enumerate}
...
\begin{Definition}[Integritäts-Ring]
  Ein Ring $\mathcal{R} = \langle R, 0, 1, +, \cdot \rangle$ heißt \blue{nullteilerfrei}, wenn
  \\[0.2cm]
  \hspace*{1.3cm}
  ...
  Ein nullteilerfreier, kommutativer Ring wird als \blue{Integritäts-Ring}
  bezeichnet.
\eoxs
\end{Definition}
...
Weise mit der Äquivalenz-Relationen \blue{verträglich} sind.  In diesem Fall nennen wir dann $\sim$ eine
\blue{Kongruenz-Relation} auf $R$.  Die formale Definition folgt.
...
Äquivalenz-Relation auf $R$.   Wir nennen $\sim$ eine \blue{Kongruenz-Relation} auf $\mathcal{R}$ 
falls zusätzlich die folgenden beiden Bedingungen erfüllt sind:
\begin{enumerate}
\item $\forall a_1, a_2, b_1, b_2 \in R: 
       \bigl(a_1 \sim a_2 \wedge b_1 \sim b_2 \rightarrow a_1 + b_1 \sim a_2 + b_2\bigr)
      $,

      die Relation $\sim$ ist also \blue{verträglich} mit der Addition auf $R$.
\item $\forall a_1, a_2, b_1, b_2 \in R: 
       \bigl(a_1 \sim a_2 \wedge b_1 \sim b_2 \rightarrow a_1 \cdot b_1 \sim a_2 \cdot b_2\bigr)
      $,

      die Relation $\sim$ ist also auch mit der Multiplikation auf $R$ \blue{verträglich}.
      \eoxs
\end{enumerate}
\end{Definition}
  
Falls $\sim$ eine Kongruenz-Relation auf einem Ring $\mathcal{R} = \langle R, 0, 1, +, \cdot \rangle$ ist,
lassen sich die Operationen ``$+$'' und ``$\cdot$'' auf den Quotienten-Raum $R/\!\sim$ der von $\sim$ auf $R$ erzeugten
...
Um nachzuweisen, dass diese Definitionen tatsächlich Sinn machen, betrachten wir vier
 Elemente $[a_1]_\sim, [a_2]_\sim, [b_1]_\sim, [b_2]_\sim \in R/\!\sim$, für welche die Gleichungen
\\[0.2cm]
\hspace*{1.3cm}
$[a_1]_\sim = [a_2]_\sim$ \quad und \quad
$[b_1]_\sim = [b_2]_\sim$ 
\\[0.2cm]
gelten.  Wir müssen zeigen, dass dann auch 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
gilt.  Damit folgt aus den Voraussetzungen $[a_1]_\sim = [a_2]_\sim$ und $[b_1]_\sim = [b_2]_\sim$, dass die Gleichungen
\\[0.2cm]
\hspace*{1.3cm}
...
\blue{Faktor-Ring} $\mathcal{R}$ modulo $\sim$ bezeichnet wird. 
...
\item $[0]_\sim$ ist das neutrale Element bezüglich der Addition, denn für alle $[a]_\sim \in R/\!\!\sim$ gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $[0]_\sim + [a]_\sim = [0 + a]_\sim = [a]_\sim$.
...
\item Die Nachweise, dass auch für den  Operator ``$\cdot$'' dass Assoziativ- und das Kommutativ-Gesetz gilt,
...
\end{enumerate}
Falls die Menge $K$ endlich ist, definieren wir die \blue{Charakteristik}
...
\item \colorbox{red}{\framebox{\colorbox{blue}{\framebox{\colorbox{yellow}{
      Die Charakteristik $\mathtt{char}(\mathcal{K})$ eines endlichen Körpers $\mathcal{K}$ ist eine Primzahl.}}}}} \eoxs
\end{enumerate}

\exerciseStar
\begin{enumerate}[(a)]
\item Zeigen Sie, dass es einen endlichen Körper $\mathcal{K}$ mit genau drei Elementen gibt.
\item Zeigen Sie, dass es einen endlichen Körper $\mathcal{K}$ mit genau vier Elementen gibt. 
\end{enumerate}
\textbf{Hinweise}:
\begin{enumerate}
\item Bei der Konstruktion eines endlichen Körpers mit genau vier Elementen sollten Sie ausnutzen,
      dass die Charakteristik eines endlichen Körpers eine Primzahl ist.
\item Die Frage, ob die von Ihnen konstruierte Struktur tatsächlich ein Körper ist, lässt sich am
...
\\[0.2cm]
der \blue{formalen Brüche}.  Die Idee dabei ist, dass wir das Paar
$\pair(x,y)$ als den Bruch $\ds\frac{x}{y}$ interpretieren.  Weiter definieren wir eine Relation 
\\[0.2cm]
\hspace*{1.3cm}
$\sim \;\subseteq Q \times Q$
\\[0.2cm]
auf $Q$ indem wir festsetzen, dass für alle Paare $\pair(x_1,y_1), \pair(x_2,y_2) \in Q$ das Folgende gilt:
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x_1, y_1) \;\sim\; \pair(x_2, y_2) \df x_1 \cdot y_2 = x_2 \cdot y_1$.
\\[0.2cm]
Diese Definition ist dadurch motiviert, dass für Brüche 
\\[0.2cm]
\hspace*{1.3cm}
...
\item Reflexivität: Für alle Paare $\pair(x,y) \in Q$ gilt nach Definition der Relation $\sim$:
      ...
      $
      \\[0.2cm]
      Da die letzte Gleichung offensichtlich wahr ist, ist die Reflexivität
...
\\[0.2cm]
ein Körper ist.  Dieser Körper wird als der von $\mathcal{R}$ erzeugte \emph{\color{blue}Quotienten-Körper} bezeichnet.


\section{Ideale und Faktor-Ringe$^*$}
Der im Folgenden definierte Begriff des \blue{Ideals} hat in der Theorie der Ringe eine ähnliche
...
Eine Teilmenge $I \subseteq R$ ist ein {\color{blue}\blue{Ideal in $\mathcal{R}$}} falls folgendes gilt:
\begin{enumerate}
\item $I \leq R$,
...
\\[0.2cm]
der zweite All-Quantor nicht nur über die Elemente aus $I$ läuft, sondern über
...
\item Wir verallgemeinern das letzte Beispiel für beliebige kommutative Ringe.
      ...
      \end{enumerate}
      Die Menge $\textsl{gen}(a)$ wird das von $a$ \blue{erzeugte Ideal} genannt.
      Ideale dieser Form werden in der Literatur als \blue{Haupt-Ideale} bezeichnet.
\item Wieder sei $\mathcal{R} = \langle R, 0, 1, +, \cdot \rangle$ ein kommutativer Ring.  Dann
      ...
      das \blue{Null-Ideal} und $R$ das \blue{Eins-Ideal}.  Diese
      beiden Ideale werden auch als die \blue{trivialen} Ideale bezeichnet.
      \eoxs
\end{enumerate}
...
\begin{Definition}[maximales Ideal]
  ...
  $I$ von $\mathcal{R}$ mit $I \not= R$ ist ein \blue{maximales Ideal} genau dann, wenn für jedes andere Ideal
...
können. 
\begin{Satz}[Faktor-Ringe maximaler Ideale sind Körper] 
  ...
  $\mathcal{R}/I$ ein Körper.
\end{Satz}

\proof  
Es ist zu zeigen, dass zu jeder Äquivalenz-Klasse $[a]_{\sim_I}$ mit $[a]_{\sim_I} \not= [0]_{\sim_I}$ ein
multiplikatives Inverses existiert.  Wir müssen also zu einer solchen Äquivalenz-Klasse
$[a]_\sim$ eine Äquivalenz-Klasse $[b]_{\sim_I}$ finden, so dass
\\[0.2cm]
\hspace*{1.3cm}
...
\noindent
\textbf{Hinweis}: Im Induktions-Schritt ist es nicht sinnvoll, von $b$ auf $b+1$ zu schließen.  Stattdessen
sollten Sie im Induktions-Schritt versuchen, die Behauptung für $b$ aus der Tatsache zu folgern,
dass die Behauptung sowohl für $b-a$ als auch für $a$ gilt. \exend
...
\section{Reflekion}
\begin{enumerate}[(a)]
\item Was ist ein Ring und was ist ein Körper?
\item Was ist eine Kongruenz-Relation?
\item Was ist ein Faktor-Ring?
\end{enumerate}
