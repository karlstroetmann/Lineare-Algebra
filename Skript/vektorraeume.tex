...
\begin{Theorem}
  Es sei $\mathcal{V} = \bigl\langle \langle V, \vec{0}, + \rangle, \cdot \bigr\rangle$ ein $\mathbb{K}$-Vektor-Raum 
  und $B_1, B_2 \subseteq V$ seien Basen von $\mathcal{V}$.
  Dann gilt $\textsl{card}(B_1) = \textsl{card}(B_2)$, zwei verschiedene Basen haben also die selbe Anzahl von
  Elementen.  Diese Anzahl bezeichnen wir als die \blue{Dimension} des Vektor-Raums.
\end{Theorem}

\noindent
Der Beweis dieses Satzes ist für eine einführende Vorlesung zu langwierig.
