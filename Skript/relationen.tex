...
der Funktion ist.  Das liegt daran, dass durch eine Funktion jedem Argument genau ein Wert zugeordnet
...
\item Die Relation $R_3$ sei definiert durch \\[0.2cm]
      ...
      Diese Relation ist rechts-eindeutig, denn die linken Seiten aller in $R_3$ auftretenden
...
dargestellten Relationen durch die in der folgenden Tabelle dargestellte Relation
gegeben:
...
\begin{enumerate}
\item[``$\Rightarrow$'']: Wir setzen voraus, dass $R$ symmetrisch ist und zeigen, dass $R^{-1} \subseteq R$
...
\blue{Asymmetrie} zwei verschiedene Begriffe sind, die nichts miteinander zu tun haben.
\eox
...
Es zeigt sich, dass für die Potenzen einer Relation und das relationale Produkt ein ähnlicher
...
\begin{Korollar}
Aus dem letzten Satz folgt sofort, dass für eine Äquivalenz-Relation $R$ auf $M$ gilt:
\\[0.2cm]
\hspace*{1.3cm}
...
\colorbox{yellow}{\blue{Quotienten-Raum}} von $M$ über $R$.  \eoxs
\end{Definition}
...
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x, y) \in\; \approx_n$ \quad g.d.w. \quad $x \;\texttt{\%}\; n = y \;\texttt{\%}\; n$.
\\[0.2cm]
...
\hspace*{1.3cm}
$\mathbb{Z}/\!\approx_n \;=\; 
  \bigl\{ \{ z \in \mathbb{Z} \mid z \;\texttt{\%}\; n = k \} \mid k \in \{0,1,\cdots, n-1\} \bigr\}$.
\\[0.2cm]
Der Quotienten-Raum besteht also aus $n$ verschiedenen Mengen.  Falls wir den Fall $n=2$ genauer
