...
\href{https://en.wikipedia.org/wiki/Equivalence_relation}{\emph{Äquivalenz-Relationen}} ein.
...
\href{https://en.wikipedia.org/wiki/Binary_relation}{\emph{binäre Relation}}.  In diesem Fall definieren
...
\begin{Definition}[links-total, rechts-total]
  Eine binäre Relation $R \subseteq M \times N$ heißt \colorbox{yellow}{\emph{links-total auf $M$},} wenn 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\begin{Definition}[Potenz einer Relation, $R^n$]
  Für eine Relation $R \subseteq M^2$ definieren wir für alle $n \in \mathbb{N}$ die \emph{Potenz} $R^n$
...
\href{https://en.wikipedia.org/wiki/Equivalence_relation}{\emph{Äquivalenz-Relation}} auf $M$
...
$R_f = \bigl\{ \pair(x,y) \in M \times M \mid f(x) = f(y) \bigr\}$ 
\\[0.2cm]
so sagen wir, dass $R_f$ die \colorbox{yellow}{\emph{von $f$ auf M erzeugte}} Äquivalenz-Relation ist.  Wir werden
...
\\[0.2cm]
Die Menge $[x]_R$ bezeichnen wir als die von $x$ erzeugte \colorbox{yellow}{\emph{Äquivalenz-Klasse}}.  \eoxs
\end{Definition}
...
\item Sei  $\pair(x,y) \in R$.  Um $[x]_R = [y]_R$ nachzuweisen zeigen wir
      ...
      \item Zeige $[x]_R \subseteq [y]_R$:
            ...
            \\[0.2cm]
            gilt.  Aus 
            $\pair(y,x) \in R$ und $\pair(x,u) \in R$ folgt wegen der Transitivität der Relation
            $R$, dass
            \\[0.2cm]
            \hspace*{1.3cm}
            $\pair(y,u) \in R$ 
      ...
      \item Zeige $[y]_R \subseteq [x]_R$:
            ...
            $\pair(y,u) \in R$ folgt wegen der Transitivität der Relation $R$ sofort
...
\item Sei nun $\pair(x,y) \not\in R$ vorausgesetzt.  Um nachzuweisen,
      ...
      \\[0.2cm]
      Aufgrund der Symmetrie von $R$ können wir $\pair(y,z) \in R$ umdrehen und haben dann
      \\[0.2cm]
      \hspace*{1.3cm}      
...
\colorbox{yellow}{\emph{Quotienten-Raum}} vom $M$ über $R$.  \eoxs
\end{Definition}
...
\begin{enumerate}
\item \colorbox{yellow}{\emph{Vollständigkeits-Eigenschaft}} 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\begin{Definition}[partielle Ordnung] \lb
  ...
  \item transitiv ist.
  \end{enumerate}
  Die Relation ist darüber hinaus eine \colorbox{yellow}{\emph{totale Ordnung auf $M$}}, wenn zusätzlich
  \\[0.2cm]
