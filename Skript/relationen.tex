...
Zum Abschluss des Kapitels f\"{u}hren wir dann 
\href{https://de.wikipedia.org/wiki/Transitive_Relation}{\emph{transitive Relationen}} und 
\href{https://en.wikipedia.org/wiki/Equivalence_relation}{\emph{\"{A}quivalenz-Relationen}} ein.
...
\href{https://en.wikipedia.org/wiki/Binary_relation}{\emph{bin\"{a}re Relation}}.  In diesem Fall definieren
wir den  \emph{Definitions-Bereich} von $R$ als 
\\[0.2cm]
\hspace*{1.3cm}
$\dom(R) := \{ x \mid \exists y \in N \colon \langle x, y \rangle \in R \}$.  
\\[0.2cm]
Entsprechend wird der \emph{Werte-Bereich} von $R$ als 
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Definition}[links-eindeutig, rechts-eindeutig] \hspace*{\fill} \\
  ...
  \colorbox{yellow}{\emph{rechts-eindeutig},} wenn folgendes gilt: \\[0.2cm]
  \hspace*{1.3cm} 
  ...
  \colorbox{yellow}{\emph{links-eindeutig},} wenn gilt: \\[0.2cm]
  \hspace*{1.3cm} 
...
\begin{Definition}[links-total, rechts-total]
  Eine bin\"{a}re Relation $R \subseteq M \times N$ hei\3t \colorbox{yellow}{\emph{links-total auf $M$},} wenn 
  \\[0.2cm]
  \hspace*{1.3cm}
  ...
  Analog nennen wir eine bin\"{a}re Relation $R \subseteq M \times N$ 
  \colorbox{yellow}{\emph{rechts-total auf $N$},} wenn 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\colorbox{yellow}{\emph{links-vollst\"{a}ndige}} Relationen  
bezeichnet.  Rechts-totale Relationen hei\3en dann \colorbox{yellow}{\emph{rechts-vollst\"{a}ndig}}. \eoxs

\begin{Definition}
Eine Relation $R \subseteq M \times N$, die sowohl links-total auf $M$ als auch rechts-eindeutig
ist, nennen wir eine \colorbox{yellow}{\emph{funktionale}} Relation auf $M$.    \eoxs
\end{Definition}
...
Zu einer  Relation $R \subseteq M \times N$ definieren wir die \colorbox{yellow}{\emph{inverse}} Relation 
...
au\3erdem sowohl links-total auf $M$ als auch rechts-total auf $N$, so nennen wir sie auch 
\colorbox{yellow}{\emph{bijektiv}}.
...
so ist das \colorbox{yellow}{\emph{relationale Produkt}} $R \circ Q$ wie folgt definiert: \\[0.2cm]
\hspace*{1.3cm}
...
Das relationale Produkt von $Q$ und $R$ wird gelegentlich auch als die \colorbox{yellow}{\emph{Komposition}} von
...
\example
Das n\"{a}chste Beispiel zeigt die Verwendung des relationalen Produkts im Kontext einer
Datenbank.  Wir nehmen an, dass die Datenbank eines Autoh\"{a}ndlers unter anderem die
...
\hline
  \end{tabular} 
\end{center}
Diese Relation k\"{o}nnte dann zur Rechnungsstellung weiterverwendet werden. \eox
...
(\ref{eq:ass0}) nachgewiesen und der Nachweis der Assoziativit\"{a}t des Kompositions-Operators
...
\begin{Satz}[Distributiv-Gesetze f\"{u}r das relationale Produkt] \lb
  ...
  sind.  Um Gleichungen der obigen Art ohne Klammern auf der rechten Seite schreiben zu k\"{o}nnen
  vereinbaren wir, dass der Kompositions-Operator $\circ$ st\"{a}rker bindet als $\cup$ und $\cap$.  
\end{Satz}
...
\begin{Definition}[Identische Relation]
  Ist $M$ eine Menge, so definieren wir die \colorbox{yellow}{\emph{identische Relation}} $\mathrm{id}_M \subseteq M \times M$
  wie folgt: \\[0.2cm]
...
Eine Relation $R \subseteq M \times M$  ist \colorbox{yellow}{\emph{reflexiv}} auf der Menge $M$ falls gilt: \\[0.2cm]
...
Eine Relation $R \subseteq M \times M$  ist \colorbox{yellow}{\emph{symmetrisch}} falls gilt: \\[0.2cm]
\hspace*{1.3cm} 
...
die Relation $R$ symmetrisch ist. \qeds

\begin{Definition}[anti-symmetrisch]
Eine Relation $R \subseteq M \times M$  ist \colorbox{yellow}{\emph{anti-symmetrisch}} falls gilt: \\[0.2cm]
\hspace*{1.3cm} 
...
$R \subseteq M^2$ als \colorbox{yellow}{\emph{asymmetrisch}} definiert, wenn 
\\[0.2cm]
\hspace*{1.3cm}
...
Eine Relation $R \subseteq M \times M$  ist \colorbox{yellow}{\emph{transitiv}} falls gilt: \\[0.2cm]
\hspace*{1.3cm} 
...
\item $R_4 := \{ \pair(m,n) \in \mathbb{N}^2 \mid \exists k\in \mathbb{N}: m\cdot k = n \}$
      ...
      F\"{u}r zwei nat\"urliche Zahlen $m$ und $n$ gilt $\pair(m,n) \in R_4$ genau dann, wenn $m$ ein
...
\begin{Definition}[Potenz einer Relation, $R^n$]
  F\"{u}r eine Relation $R \subseteq M^2$ definieren wir f\"{u}r alle $n \in \mathbb{N}$ die \emph{Potenz} $R^n$
...
\begin{Satz}[Potenz-Gesetz des relationalen Produkts] \hspace*{\fill} \\
  Es sei $R \subseteq M \times M$.  F\"{u}r beliebige ganze Zahlen $k,l \in \mathbb{N}$ gilt:
  \[ R^k \circ R^l = R^{k+l}. \]
\end{Satz}
...
\begin{Definition}[transitiver Abschluss einer Relation, $R^+$]\hspace*{\fill} \\
  ...
  Wir definieren den 
  \href{https://en.wikipedia.org/wiki/Transitive_closure}{\emph{transitiven Abschluss}} der Relation
  ...
  \\[0.2cm]
  definiert.  Etwas weniger formal (aber daf\"{u}r anschaulicher) k\"{o}nnen wir auch schreiben
  \\[0.2cm]
  \hspace*{1.3cm}
...
\item $R^+$   ist die bez\"{u}glich der Inklusions-Ordnung $\subseteq$ kleinste Relation
      ...
      Anders ausgedr\"{u}ckt: Ist $T$ eine transitive Relation mit $R \subseteq T$, so
...
\]
Nach der Definition der Menge $\bigcup\limits_{n\in\mathbb{N}^*} R^n$ gibt es dann positive nat\"{u}rliche Zahlen $k,l\in\mathbb{N}^*$, so dass
...
Nachweis zu f\"{u}hren, zeigen wir zun\"{a}chst, dass f\"{u}r alle positiven
...
\href{https://en.wikipedia.org/wiki/Equivalence_relation}{\emph{\"{A}quivalenz-Relation}} auf $M$
genau dann, wenn folgende Bedingungen erf\"{u}llt sind: 
\begin{enumerate}
...
\\[0.2cm]
definieren.  Falls $x \approx_n y$ gilt, so sagen wir, dass \colorbox{yellow}{$x$ und $y$ kongruent modulo $n$} sind.
...
\\[0.2cm]
Wir zeigen, dass die Relation $\approx_n$ f\"{u}r $n \in \mathbb{N}^*$ eine \"{A}quivalenz-Relation auf
$\mathbb{Z}$ definiert. 
...
\item Um zu zeigen, dass $\approx_n$ reflexiv ist, m\"{u}ssen wir nachweisen, dass f\"{u}r alle
      ...
      $0\cdot n = 0 = x - x$. 
      \\[0.2cm]
      Damit ist die Reflexivit\"{a}t nachgewiesen. {\color{green}{$\surd$}}
...
\item Zum Nachweis der Transitivit\"{a}t von $\approx$ nehmen wir an, dass
      ...
      \\[0.2cm]
      nachgewiesen und folglich die Transitivit\"{a}t von $\approx_n$ gezeigt. {\color{green}{$\surd$}} 
      \qeds
\end{enumerate}
\pagebreak 

\exercise
Es sei $n \in \mathbb{N}^*$. Beweisen Sie, dass f\"{u}r alle ganzen Zahlen $x$ und $y$ die Beziehung
\\[0.2cm]
\hspace*{1.3cm}
...
$R_f = \bigl\{ \pair(x,y) \in M \times M \mid f(x) = f(y) \bigr\}$ 
\\[0.2cm]
so sagen wir, dass $R_f$ die \colorbox{yellow}{\emph{von $f$ auf M erzeugte}} \"{A}quivalenz-Relation ist.  Wir werden
sp\"{a}ter sehen, dass es zu jeder  \"{A}quivalenz-Relation eine Funktion gibt, die diese
\"{A}quivalenz-Relation erzeugt. \eoxs 
...
Staatsb\"{u}rgerschaft haben.  \eoxs
...
\\[0.2cm]
Die Menge $[x]_R$ bezeichnen wir als die von $x$ erzeugte \colorbox{yellow}{\emph{\"{A}quivalenz-Klasse}}.  \eoxs
\end{Definition}
...
\remark
Da f\"{u}r $x,y\el M$ entweder $\pair(x,y) \in R$ oder $\pair(x,y) \not\in{R}$ gilt, zeigen die
...
\item Wir haben $x \el [x]_R$ genau dann, wenn 
      ...
      \"{a}quivalent zu $\pair(x,x) \in R$.
      Die Eigenschaft $\pair(x,x) \in R$ folgt aber unmittelbar aus der Reflexivit\"{a}t von $R$. {\color{green}{$\surd$}}
\item Sei  $\pair(x,y) \in R$.  Um $[x]_R = [y]_R$ nachzuweisen zeigen wir
      ...
      \item Zeige $[x]_R \subseteq [y]_R$:
            ...
            $\pair(x,y) \in R$ folgt wegen der Symmetrie der Relation $R$, dass auch
            \\[0.2cm]
            \hspace*{1.3cm}
            $\pair(y,x) \in R$ 
            \\[0.2cm]
            gilt.  Aus 
            $\pair(y,x) \in R$ und $\pair(x,u) \in R$ folgt wegen der Transitivit\"{a}t der Relation
            $R$, dass
            \\[0.2cm]
            \hspace*{1.3cm}
            $\pair(y,u) \in R$ 
      ...
      \item Zeige $[y]_R \subseteq [x]_R$:
            ...
            $\pair(y,u) \in R$ folgt wegen der Transitivit\"{a}t der Relation $R$ sofort
...
\item Sei nun $\pair(x,y) \not\in R$ vorausgesetzt.  Um nachzuweisen,
      ...
      \\[0.2cm]
      Aufgrund der Symmetrie von $R$ k\"{o}nnen wir $\pair(y,z) \in R$ umdrehen und haben dann
      \\[0.2cm]
      \hspace*{1.3cm}      
...
\colorbox{yellow}{\emph{Quotienten-Raum}} vom $M$ \"{u}ber $R$.  \eoxs
\end{Definition}
...
Staatsb\"{u}rgerschaft besitzen.  \eoxs

\example
Auf der Menge $\mathbb{Z}$ definieren wir f\"{u}r $n\in\mathbb{N}$ und $n \geq 2$ die Relation $\approx_n$ wie folgt:
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x, y) \in\; \approx_n$ \quad g.d.w. \quad $x \;\mathtt{\%}\; n = y \;\mathtt{\%}\; n$.
\\[0.2cm]
Wir haben bereits fr\"{u}her gezeigt, dass diese Relation eine \"{A}quivalenz-Relation ist.  F\"{u}r den Quotienten-Raum, der von dieser Relation erzeugt wird, gilt
\\[0.2cm]
\hspace*{1.3cm}
$\mathbb{Z}/\!\approx_n \;=\; 
...
betrachten, so stellen wir fest, dass $\mathbb{Z}/\!\approx_2$ genau zwei Mengen enth\"{a}lt:  Die Menge
der geraden Zahlen und die Menge der ungeraden Zahlen.  
\eoxs


\begin{Definition}[Partition] 
Ist ${\cal P} \subseteq 2^M$ eine Menge von Teilmengen von $M$, so sagen wir, dass ${\cal P}$ eine 
\href{https://de.wikipedia.org/wiki/Partition_(Mengenlehre)}{\emph{Partition}} von $M$ ist, falls
$\mathcal{P}$ die folgenden Eigenschaften hat: 
\begin{enumerate}
\item \colorbox{yellow}{\emph{Vollst\"{a}ndigkeits-Eigenschaft}} 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\forall x \el M : \exists K \el {\cal P} : x \el K$,

      jedes Element aus $M$ findet sich  in einer Menge aus ${\cal P}$ wieder.
\item \colorbox{yellow}{\emph{Separations-Eigenschaft}}
      \\[0.2cm]
      \hspace*{1.3cm}
...
\end{enumerate}
Gelegentlich wird eine Partition einer Menge $M$ auch als \colorbox{yellow}{\emph{Zerlegung}} von $M$
bezeichnet. \eoxs
\end{Definition}
...
\begin{Definition}[partielle Ordnung] \lb
  Eine Relation $R \subseteq M \times M$  ist eine 
  \colorbox{yellow}{\emph{partielle Ordnung (im Sinne von $\leq$) auf $M$}} genau dann, wenn die Relation $R$
  \begin{enumerate}
  \item reflexiv auf $M$,
  \item anti-symmetrisch und
  \item transitiv ist.
  \end{enumerate}
  Die Relation ist dar\"{u}ber hinaus eine \colorbox{yellow}{\emph{totale Ordnung auf $M$}}, wenn zus\"{a}tzlich
  \\[0.2cm]
...
\remark
Eine totale Ordnung wird gelegentlich auch als \colorbox{yellow}{\emph{lineare Ordnung}} bezeichnet. \eoxs
...
\item Anti-Symmetrie: Zu zeigen ist 
      ...
      sofort auf $x = y$ schlie\3en.  Damit ist die Anti-Symmetrie gezeigt. {\color{green}{$\surd$}}
\item Transitivit\"{a}t: Zu zeigen ist 
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \\[0.2cm]
      Damit haben wir die Transitivit\"{a}t nachgewiesen.  {\color{green}{$\surd$}}
\end{enumerate}
...
\noindent
Wir schlie\3en damit unseren Ausflug in die Mengenlehre ab und verweisen f\"{u}r weitere
