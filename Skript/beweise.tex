...
zusätzlich $q > 0$ ist.  Dabei k\"{o}nnen
...
\href{https://de.wikipedia.org/wiki/Vollständige_Induktion}{vollst\"{a}ndige Induktion}. 
...
dieser Argumentation schlie\3en wir insgesamt, dass $F(n)$ f\"{u}r jede beliebige natürliche Zahl richtig ist.  Diese
...
Wir definieren eine \emph{Menge} $M$ als die Menge\footnote{
  Wir werden den Begriff der \emph{Menge} im nächsten Kapitel formal definieren.  
} 
...
für $n$ die Zahl $n_0$ einsetzen, dann k\"{o}nnen wir dann folgern, dass
\\[0.2cm]
\hspace*{1.3cm}
$F\bigl((n_0-1) +1\bigr)$
\\[0.2cm] 
...
\\[0.2cm]
gilt.  Die 
Wie lautet die Formel im Falle $q=1$? \exend
