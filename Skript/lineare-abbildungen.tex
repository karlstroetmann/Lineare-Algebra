...
führen wir lineare Abbildungen ein und zeigen, wie diese durch
\href{https://de.wikipedia.org/wiki/Matrix_(Mathematik)}{Matrizen} dargestellt 
werden können.  Wir definieren Addition und Multiplikation von Matrizen und zeigen, dass bestimmte Matrizen
bezüglich der Multiplikation ein {\emph{\color{blue}Inverses}} besitzen.  Wir führen sogenannte
\href{https://de.wikipedia.org/wiki/Elementarmatrix}{Elementar-Matrizen} ein und demonstrieren, wie
sich mit Hilfe von Elementar-Matrizen das Inverse einer Matrix berechnen lässt.   

Im Rest dieses Abschnittes bezeichnet $\mathbb{K}$ einen beliebigen Körper.  In den Anwendungen wird es sich dabei
meistens um den Körper $\mathbb{R}$ der reellen Zahlen oder den Körper $\mathbb{C}$ der komplexen Zahlen handeln.  
...
\item $\forall \vec{x}, \vec{y} \in V: f(\vec{x} + \vec{y}) = f(\vec{x}) + f(\vec{y})$,
      ...
      eine lineare Abbildung ist also mit der Addition verträglich.
\item $\forall \alpha \in \mathbb{K}: \forall \vec{x} \in V: f(\alpha \cdot \vec{x}) = \alpha \cdot f(\vec{x})$,

      eine lineare Abbildung ist also auch mit der Skalar-Multiplikation verträglich.
\end{enumerate}
...
\\[0.2cm]
\blue{punktweise}, das heißt für $f,g \in \mathcal{L}(V, W)$ definieren wir die
lineare Abbildung $f\blue{+}g$ indem wir fordern, dass für alle $\vec{x} \in V$
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
setzen.  Schließlich definieren wir eine \blue{Skalar-Multiplikation} von Elementen des
Körpers  $\mathbb{K}$ mit linearen Abbildung aus $\mathcal{L}(V,W)$.  Diese Skalar-Multiplikation bezeichnen
...
Vektor-Raum $W$ auch als ein {\blue{Vektor-Raum-Homomorphismus}} oder kürzer als ein
{\blue{Homomorphismus}} bezeichnet.  Ist die Abbildung $f$ außerdem noch injektiv, so heißt $f$ ein
...
{\blue{Operator auf $V$}} bezeichnet.  Wir definieren zur Abkürzung
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Weniger formal aber dafür intuitiver könnten wir auch schreiben
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Definition}[Kern]
  Es seien $V$ und $W$ zwei $\mathbb{K}$-Vektor-Räume und $f:V \rightarrow W$ sei ein lineare Abbildung.  Dann definieren wir 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\begin{Satz}
  Es seien $V$ und $W$ $\mathbb{K}$-Vektor-Räume und $f:V \rightarrow W$ sei eine lineare Abbildung.
...
\begin{enumerate}
\item[``$\Rightarrow$'':] Wir nehmen zunächst an, dass $f$ injektiv ist.  Da $f$ eine lineare Abbildung ist, gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(\vec{0}) = \vec{0}$
      \\[0.2cm]
      damit gilt sicher $\vec{0} \in \textsl{Kern}(f)$.  Sei nun zusätzlich $\vec{y} \in \textsl{Kern}(f)$, es
      gelte also 
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(\vec{y}) = \vec{0}$.
...
\item[``$\Leftarrow$'':] Wir setzen jetzt $\textsl{Kern}(f) = \bigl\{ \vec{0} \bigr\}$ voraus.  Falls für $\vec{x}, \vec{y} \in V$
      nun
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(\vec{x}) = f(\vec{y})$
...
Es seien $V$ und $W$ $\mathbb{K}$-Vektor-Räume und $f:V \rightarrow W$ sei eine lineare Abbildung.  Dann 
definieren wir
\\[0.2cm]
\hspace*{1.3cm}
\colorbox{blue}{\framebox{\colorbox{yellow}{
...
Das Bild und der Kern einer linearen Abbildung sind über den
\href{https://de.wikipedia.org/wiki/Rangsatz}{Dimensions-Satz} verknüpft.

\begin{Satz}[Dimensions-Satz, \href{https://de.wikipedia.org/wiki/Rangsatz}{Rang-Satz}]
  Es seien $V_1$ und $V_2$ endlich-dimensionale $\mathbb{K}$-Vektor-Räume und die Funktion $f: V_1 \rightarrow V_2$ sei eine lineare Abbildung.
  Dann gilt
  \\[0.2cm]
  \hspace*{1.3cm}
  \colorbox{blue}{\framebox{\colorbox{yellow}{
...
\item Wir zeigen: Die Menge $\bigl\{ f(\vec{y}_1), \cdots, f(\vec{y}_n) \bigr\}$ ist linear unabhängig.
      ...
      eine Basis von $V_1$ ist, ist diese Menge insbesondere linear unabhängig.  Also
...
\section{Matrizen}
Es seien $V$ und $W$ $\mathbb{K}$-Vektor-Räume und $f$ sei eine lineare Abbildung.  Weiter sei
...
für alle $i \in \{1,\cdots,n\}$ und $j \in \{1,\cdots,m\}$ schreiben wir als \blue{$\mathbb{K}^{n \times m}$}.

\begin{Definition}[$\mathcal{M}(f)$]
Es gelte:
\begin{enumerate}
\item $V$ und $W$ sind $\mathbb{K}$-Vektor-Räume,
...
\\[0.2cm]
schreiben.  Diese Schreibweise ist aber sehr schwerfällig.  Wir
...
$x_i \in \mathbb{K}$ für alle $i\in\{1,\cdots,n\}$ definiert.  Diese \blue{Listen-Schreibweise} ist wesentlich
...
\\[0.2cm]
Diese Gleichungen zeigen, dass die Skalar-Multiplikation für Matrizen mit der Matrix-Vektor-Multiplikation   
verträglich ist.
...
Operationen zu einem $\mathbb{K}$-Vektor-Raum wird.  Das neutrale Element bezüglich der Addition ist die
\blue{Null-Matrix}, deren sämtliche Komponenten den Wert $0$ haben.

\remark
Sind $V$ und $W$ zwei endlich-dimensionale $\mathbb{K}$-Vektor-Räume mit Basen
...
\\[0.2cm]
\hspace*{1.3cm}
$f(\vec{x}) := A \cdot \vec{x}$ \quad für alle~$\vec{x} \in \mathbb{K}^l$,
 \quad und \quad
 $g(\vec{y}) := B \cdot \vec{y}$ \quad für alle~$\vec{y} \in \mathbb{K}^m$,
 \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$h(\vec{x}) := (g \circ f)(\vec{x}) := g(f(\vec{x}))$ \quad für alle~$\vec{x} \in \mathbb{K}^l$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds c_{i,j} := \sum\limits_{k=1}^m b_{i,k} \cdot a_{k,j}$ \quad für alle $i \in \{1,\cdots,n\}$ und $j \in\{1,\cdots,l\}$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds c_{i,j} := \sum\limits_{k=1}^m b_{i,k} \cdot a_{k,j}$ \quad für alle $i \in \{1,\cdots,n\}, j \in\{1,\cdots,l\}$,
\\[0.2cm]
gegeben sind.

\remark
Sind $V_1$, $V_2$ und $V_3$ $\mathbb{K}$-Vektor-Räume und gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\eoxs
\end{Definition}
Damit sind wir in der Lage, für alle $n \in \mathbb{N}$ eine {\blue{Einheits-Matrix}} $\mathrm{E}_n$ zu
...
\remark
In der Literatur werden invertierbare Matrizen auch als \blue{reguläre} oder \blue{nicht-singuläre}
Matrizen bezeichnet.  Dementsprechend heißt eine Matrix, die nicht invertierbar ist, dann eine
\blue{singuläre} Matrix.

\remark
Wenn es für eine quadratische Matrix $A$ eine Matrix $B$ gibt, so dass $A \cdot B = \mathrm{E}_n$ gilt, dann kann gezeigt werden, dass daraus
...
$n$ Gleichungen und $n$ Variablen lässt sich in der Form $A \cdot \vec{x} = \vec{b}$ schreiben, wobei
...
das Lösen der Gleichung $A \cdot \vec{x} = \vec{b}$  auf eine Matrix-Multiplikation zurückführen.
Da das Inverse $A^{-1}$ nicht von dem Vektor $\vec{b}$ abhängt, ist dies insbesondere dann ein
...
\begin{Definition}[Elementar-Matrizen]
  ...
  \item Für $k\not=l$ wird die $n \times n$ {\blue{Zeilen-Additions-Matrix}} $\mathrm{ZA}_n(k,l,\alpha)$ durch
        ...
        \right).
        $       
\item Die $n \times n$ {\blue{Zeilen-Permutations-Matrix}} $\mathrm{ZP}_n(k,l)$ wird für $k \not= l$ durch die Gleichung 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Die $n \times n$ {\blue{Zeilen-Multiplikations-Matrix}} $\mathrm{ZM}_n(k, \alpha)$ wird für $\alpha \not= 0$ durch die Gleichung
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item $\bigl(\mathrm{ZM}_n(k,\alpha)\bigr)^{-1} = \mathrm{ZM}_n(k,\alpha^{-1})$ \quad falls $\alpha \not= 0$,
      ...
      Gleichung  $\alpha^{-1} \cdot \alpha = 1$ insgesamt nicht verändert.  
\end{enumerate}
...
Für die Transformation der Matrix $A \in \mathbb{K}^{n\times n}$ nehmen wir an, dass
\\[0.2cm]
\hspace*{1.3cm}
 $A = \bigl(a_{i,j}\bigr)_{i=1,\cdots,n \atop j=1,\cdots,n}$
\\[0.2cm]
gilt.  Die Transformation verläuft in $n$ Schritten, wobei wir im $i$-ten
...
$j \in \{i, \cdots, n\}$ für die $|a_{j,i}|$ maximal wird.  Theoretisch würde es bei exakter Rechnung an dieser Stelle
reichen, einfach irgendeine Zeile $j \in \{i, \cdots, n\}$ zu wählen, für die $a_{j,i} \not= 0$ 
...
beim Rechnen mit \href{https://de.wikipedia.org/wiki/Gleitkommazahl}{Fließkommazahlen} häufig Werte
auftreten, die bei exakter Rechnung den Wert $0$ haben 
müssten, die aber bei Fließkomma-Rechnung stattdessen einen sehr kleinen Wert wie etwa 
$2 \cdot 10^{-16}$ annehmen.  Wäre nun $a_{j,i}$ bei exakter Rechnung $0$ und würde durch einen
Rundungs-Fehler stattdessen ein sehr kleiner Wert übrig bleiben, so würden wir in der folgenden Rechnung
mit diesem Wert weiter rechnen.  Insbesondere würden wir durch diesen sehr kleinen Wert später teilen.
Das macht aber keinen Sinn, denn wenn wir korrekt gerechnet hätten, würden wir an der Stelle dann durch $0$ teilen, was
gar nicht möglich ist.  Im Endeffekt würden wir dann völlig unsinnige Werte als Lösung erhalten.
...
\item Die Funktion \texttt{inverse} bekommt als Argument eine quadratische Matrix \texttt{A}, die 
      ...
      $a_{i,j}$ der Matrix $\mathtt{A}$ kann dann mittels des Ausdrucks $\mathtt{A}[i][j]$ zugegriffen werden.
\item Zunächst wird in Zeile 2 mit Hilfe des Operators ``\texttt{\#}''
      die Dimension $n$ der Matrix berechnet: Dies ist gerade die Länge der
      Liste \texttt{A}.
...
\item Im $i$-ten Durchlauf der Schleife in Zeile 4 wird erreicht, dass die $i$-te Spalte von
      ...
      \item Falls nach der Vertauschung von Zeile $i$ und Zeile $r$ die Komponente
            ...
            $\mathtt{A}[i][i] = 0$ ist, wird als Ergebnis der undefinierte Wert $\Omega$ zurück gegeben. 
      \item Ansonsten teilen wir in Zeile 13 des Programms die $i$-Zeile von $\mathtt{A}$ durch
            $\mathtt{A}[i][i]$.  Anschließend hat $A[i][i]$ dann offenbar den Wert $1$. 
            Die selbe Operation wird in Zeile 14 auf die Matrix $\mathtt{E}$ angewendet.
      \item In Zeile 19 des Programms subtrahieren wir das $\mathtt{A}[k][i]$-fache
            der $i$-ten Zeile von der $k$-ten Zeile für alle $k \not= i$.  Da die $i$-te Zeile an der Position $i$
      ...
      beendet wurde.  Gleichzeitig enthält dann die Variable $\mathtt{E}$ das Inverse der
      ursprünglich gegebenen Matrix $\mathtt{A}$.
\end{enumerate}
...
\texttt{pivot}.  
\begin{enumerate}
\item Zunächst implementiert die Funktion \texttt{delta} das Kronecker-Delta, es gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{delta}(i,j) = \delta_{i,j} := \left\{
                  ...
                  \right.
      $
\item Mit Hilfe des Kronecker-Deltas wird dann die Funktion \texttt{identity}, welche die Einheits-Matrix $\mathrm{E}_n$
...
\item Die Funktion \texttt{pivot} sucht das betragsmäßig größte Element aus der Menge
      \\[0.2cm]
      \hspace*{1.3cm}
      $\ds\bigl\{ \mathtt{A}[j][i] \mid j \in \{i,\cdots,n\}\bigr\}$
      \\[0.2cm]
      und gibt die Zeile $\mathtt{r}$ zurück, an der dieses Element gefunden wird.  Falls
      $\mathtt{pivot}(A, n, i) = r$ ist, gilt also
      \\[0.2cm]
      \hspace*{1.3cm}
      $\forall j \in \{i,\cdots,n\}: |\mathtt{A}[r][i]| \geq |\mathtt{A}[j][i]|$.
\end{enumerate}
...
$
\\[0.2cm]
zu berechnen.  Dazu wird zunächst die Variable $\mathtt{E}$ mit der Einheits-Matrix $\mathrm{E}_3$
...
Die Programmiersprache \textsc{SetlX} unterstützt reelle Vektoren und reelle Matrizen nativ.  Um die oben
angegeben Matrix \texttt{A} zu invertieren, hätten wir die Matrix \texttt{A} in \textsc{SetlX} auch als
\begin{verbatim}
    A := << <<1 2 3>>
            <<2 3 1>>
            <<3 1 2>>
         >>;
\end{verbatim}
definieren können.  Dann berechnet der Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{B := A ** -1;}
\\[0.2cm]
das Inverse der Matrix \texttt{A}, allerdings rechnet \textsc{SetlX} in diesem Fall mit
Fließkomma-Zahlen, so dass bei der Berechnung von $\mathtt{A}^{-1}$ Rundungs-Fehler auftreten.
...
>>
\end{verbatim}
Es lässt sich zeigen, dass bei einer \blue{exakter} Berechnung des Inversen einer $n \times n$ Matrix
$A$, die aus rationalen Zahlen besteht, die Rechenzeit zur Berechnung von $A^{-1}$ exponentiell mit
der Dimension $n$ wächst, so dass eine exakte Rechnung für große Werte von $n$ nicht
praktikabel ist.  Der Grund dafür ist, dass die Zähler und Nenner der Brüche, mit denen wir es
dann zu tun haben, sehr stark anwachsen.  Daher rechnen alle Programmiersprachen, die Pakete zur Linearen
Algebra enthalten, mit Fließkomma-Zahlen.
