...
dass die Matrix $A$ in diesem Fall nicht invertierbar ist.  Falls wir stattdessen einen Index $j \in \{1,\cdots,n\}$ 
...
\item Das Prinzip der Berechnung der inversen Matrix $\texttt{A}^{-1}$ ist, dass $\texttt{A}$
      mittels elementarer Zeilen-Umformungen in die Einheits-Matrix $\textrm{E}_n$ überführt wird.
      Gleichzeitig werden diese Umformungen auf die Einheits-Matrix $\textrm{E}_n$ angewendet
      und am Ende enthält dann die umgeformte Einheits-Matrix das Inverse der Matrix $\texttt{A}$.
...
\item Im $i$-ten Durchlauf der Schleife in Zeile 4 wird erreicht, dass die $i$-te Spalte von
      $\texttt{A}$ mit dem $i$-ten Einheits-Vektor $\vec{e}_i$ übereinstimmt.
      \begin{enumerate}
      \item Zunächst suchen wir mit der in Zeile $5$ aufgerufenen Funktion \texttt{pivot}
            nach der Zeile $r$, für die der Betrag $|\texttt{A[}i\texttt{][}r\texttt{]}|$ unter allen 
            $r \in \{i, \cdots,n \}$ maximal wird.  Diese Zeile wird dann in Zeile 6 des Programms mit der $i$-ten Zeile
            vertauscht.  Anschließend wird in Zeile 7 dieselbe Operation auf der Matrix $\texttt{E}$ ausgeführt.
      ...
      \item Falls nach der Vertauschung von Zeile $i$ und Zeile $r$ die Komponente
            ...
            $\texttt{A[}i\texttt{][}i\texttt{]} = 0$ ist, wird als Ergebnis der undefinierte Wert $\Omega$ zurück gegeben. 
      \item Ansonsten teilen wir in Zeile 13 des Programms die $i$-Zeile von $\texttt{A}$ durch
            $\texttt{A[}i\texttt{][}i\texttt{]}$.  Anschließend hat $\texttt{A[}i\texttt{][}i\texttt{]}$ dann offenbar den Wert $1$. 
            Dieselbe Operation wird in Zeile 14 auf die Matrix $\texttt{E}$ angewendet.
      \item In Zeile 19 des Programms subtrahieren wir das $\texttt{A[}k\texttt{][}i\texttt{]}$-fache
            ...
            $\texttt{A[}i\texttt{][}k\texttt{]}$ für $k \not= i$ gleich $0$ sind.
            Dieselbe Operation wird in Zeile 20 auf die Matrix $E$ angewendet.
      \end{enumerate}
      Nachdem $i$ den Wert $n$ erreicht hat, ist die ursprüngliche Matrix $\texttt{A}$ zur
      Einheits-Matrix reduziert worden, sofern die Funktion nicht vorher in Zeile 9 ergebnislos  
      beendet wurde.  Gleichzeitig enthält dann die Variable $\texttt{E}$ das Inverse der
      ursprünglich gegebenen Matrix $\texttt{A}$.
\end{enumerate}
...
\item Die Funktion \texttt{pivot} sucht das betragsmäßig größte Element aus der Menge
      ...
      \\[0.2cm]
      und gibt die Zeile $\texttt{r}$ zurück, an der dieses Element gefunden wird.  Falls
...
$
\\[0.2cm]
zu berechnen.  Dazu wird zunächst die Variable $\texttt{E}$ mit der Einheits-Matrix $\textrm{E}_3$
...
$
 \\[0.2cm]
\begin{enumerate}
\item Im ersten Schritt suchen wir die Zeile der Matrix $\texttt{A}$, für die in der ersten Spalte
      das größte Element steht.  Im vorliegenden Fall ist das die dritte Zeile, denn es gilt $\texttt{A[3][1]} = 3$
      und alle anderen Einträge in der ersten Spalte sind kleiner als $3$.  Daher vertauschen wir die
      erste Zeile mit der dritten Zeile und finden für die Matrizen $\texttt{A}$ und $\texttt{E}$
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{A} = \left(
  \begin{array}[c]{lll}
...
\item Im nächsten Schritt normalisieren wir die erste Zeile, indem wir durch $\texttt{A[1][1]}$
...
$
\\[0.2cm]
      Unser nächstes Ziel ist es, alle Einträge $\texttt{A[}i\texttt{][1]}$ in der ersten Spalte, für die 
...
$\texttt{E} = \left(
  ...
  \end{array}\right)
$
\item Im nächsten Schritt geht es darum, die Matrix $\texttt{A}$ durch Addition eines geeigneten
      Vielfachen der zweiten Zeile zur ersten Zeile so zu verändern, dass $\texttt{A[2,1]} = 0$
...
$\texttt{E} = \left(
  ...
  \end{array}\right)
$
\item Anschließend subtrahieren wir $\ds\frac{5}{3}$ der zweiten Zeile von der dritten Zeile und finden
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{A} = \left(
  \begin{array}[c]{rrr}
...
Fließkomma-Zahlen, so dass bei der Berechnung von $\texttt{A}^{-1}$ Rundungs-Fehler auftreten.
