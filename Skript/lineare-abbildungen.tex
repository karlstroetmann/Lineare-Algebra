...
\end{Satz}
Der Beweis dieses Satzes ist nur mit Hilfe des Basis-Austausch-Satzes möglich, den wir aus Zeitgründen nicht
behandelt haben.
...
\\[0.2cm]
\hspace*{1.3cm}
$c_{i,j} := \sum\limits_{k=1}^m b_{i,k} \cdot a_{k,j}$ \quad für alle $i \in \{1,\cdots,n\}$ und $j \in\{1,\cdots,l\}$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$c_{i,j} := \sum\limits_{k=1}^m b_{i,k} \cdot a_{k,j}$ \quad für alle $i \in \{1,\cdots,n\}, j \in\{1,\cdots,l\}$,
\\[0.2cm]
gegeben sind.
...
$n$ Gleichungen und $n$ Variablen lässt sich in der Form $A \cdot \vec{x} = \vec{x}$ schreiben, wobei $A \in \mathbb{K}^{n \times n}$ ist.
...
\href{https://de.wikipedia.org/wiki/Elementarmatrix}{Elementar-Matrizen}.  Die  
...
beim Rechnen mit \href{https://de.wikipedia.org/wiki/Gleitkommazahl}{Fließkomma-zahlen} häufig Werte
...
>>
\end{verbatim}
Leider lässt sich zeigen, dass bei einer exakter Berechnung des Inversen einer $n \times n$ Matrix
$A$, die aus rationalen Zahlen besteht, die Rechenzeit zur Berechnung von $A^{-1}$ exponentiell mit
der Dimension $n$ wächst, so dass eine exakte Rechnung für größere Werte von $n$ nicht
