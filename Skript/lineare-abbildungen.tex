...
\begin{Definition}[Kern]
  ...
  ${\color{blue}\textsl{Kern}(f)} := \bigl\{ \vec{x} \in V \mid f(\vec{x}) = \vec{0} \bigr\}$.}}}}}
  \\[0.2cm] 
...
\hspace*{1.3cm}
\colorbox{red}{\framebox{\colorbox{blue}{\framebox{\colorbox{yellow}{
${\color{blue}\textsl{Bild}(f)} := \bigl\{ f(\vec{x}) \mid \vec{x} \in V \bigr\}$}}}}}
\\[0.2cm]
...
das L\"{o}sen der Gleichung $A \cdot \vec{x} = \vec{b}$  auf eine Matrix-Multiplikation zur\"{u}ckf\"{u}hren.
Da das Inverse $A^{-1}$ nicht von dem Vektor $\vec{b}$ abh\"{a}ngt, ist dies insbesondere dann ein
...
\begin{Definition}[Elementar-Matrizen]
  ...
  \item F\"{u}r $k\not=l$ wird die $n \times n$ {\emph{\color{blue}Zeilen-Additions-Matrix}} $\mathrm{ZA}_n(k,l,\alpha)$ durch
        ...
        \right).
        $       
\item Die $n \times n$ {\emph{\color{blue}Zeilen-Permutations-Matrix}} $\mathrm{ZP}_n(k,l)$ wird f\"ur $k \not= l$ durch die Gleichung 
      \\[0.2cm]
      \hspace*{1.3cm}
...
>>
\end{verbatim}
Leider l\"asst sich zeigen, dass bei einer exakter Berechnung des Inversen einer $n \times n$ Matrix
