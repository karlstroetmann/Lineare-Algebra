...
\item Einerseits gilt 
      ...
      $A_6$ und $B_6$ leer, während die andere das Paar $\pair(k,k+1)$ enthält, formal gilt
      \\[0.2cm]
      \hspace*{1.3cm}
...
Berechnung der oben angegebenen Leibniz-Formel zur Berechnung der Determinante.  
\begin{enumerate}
\item Die Funktionen $\texttt{inv}$ erhält als Eingabe eine Permutation, die als Liste $\texttt{L}$ dargestellt
      ist und berechnet die Menge der Inversionen dieser Permutation.  Die Implementierung von $\texttt{inv}$
      setzt die mathematische Definition der Funktion $\mathtt{inv}$ unmittelbar um.
\item Die Funktionen $\texttt{sgn}$ erhält ebenfalls als Eingabe eine Permutation, die als Liste $\texttt{L}$ dargestellt
      ist und berechnet das Vorzeichen dieser Permutation.
\item Die Funktion $\texttt{allPermutations}$ erhält als Eingabe eine Menge $\texttt{S}$ von Zahlen und
      berechnet die Menge aller der Listen, in der jedes Element der Menge $\texttt{S}$ genau einmal auftritt.
      Diese Funktion wird später mit dem Argument $\{1,\cdots, n\}$ aufgerufen, um alle Permutationen der
      natürlichen Zahlen von $1$ bis $n$ zu berechnen.

      Falls die Menge $\texttt{S}$ leer ist, ist die leere Liste die einzige Permutation der Elemente aus $\texttt{S}$.

      Falls die Menge $\texttt{S}$ nicht leer ist, beginnt eine Permutation $\texttt{R}$ der Elemente aus
      $\texttt{S}$ mit einer beliebigen Zahl $\texttt{k}$ aus $\texttt{S}$.  Auf diese Zahl $\texttt{k}$ folgt
      dann eine Liste, die rekursiv aus den restlichen Zahlen der Menge $\texttt{S}$ gebildet wird.
\item Die Funktion $\texttt{determinant}$ erhält als Eingabe eine positive natürliche Zahl $\texttt{n}$ und
      liefert als Ergebnis einen String $\texttt{det}$ zurück, der den Inhalt der Leibniz-Formel als String wiedergibt. 
      Dabei wird die Definition der Determinante nach Leibniz unmittelbar umgesetzt:
      \begin{enumerate}
      \item Zunächst wird die Menge $\texttt{Sn}$ aller Permutationen der Zahlen von $1$ bis $\texttt{n}$
            berechnet.  Diese Permutationen werden als Listen dargestellt. 
      \item Anschließend wird für jede Permutation $\texttt{L}$ aus der Menge $\texttt{Sn}$ das Vorzeichen berechnet.
            Je nach dem, ob dieses Vorzeichen den Wert $1$ oder $-1$ hat, wird der String $\texttt{sign}$ auf
            den Wert $\texttt{\symbol{34} + \symbol{34}}$ oder $\texttt{\symbol{34} - \symbol{34}}$ gesetzt.
      \item Erklärungsbedürftig ist eventuell die Funktion $\texttt{join}$:  Diese wird im Allgemeinen mit
            zwei Argumenten in der Form
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{join}(L, s)$
            \\[0.2cm]
            aufgerufen.  Dabei ist $L$ eine Liste von Strings und $s$ ist ein einzelner String.  Der Aufruf von
            $\texttt{join}$ verkettet die Strings aus $L$ und fügt dabei jeweils zwischen zwei Strings aus $L$
            den String $s$ ein.  Beispielsweise liefert der Ausdruck
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{join([\symbol{34}a\symbol{34}, \symbol{34}b\symbol{34}, \symbol{34}c\symbol{34}], \symbol{34}+\symbol{34})}$
            \\[0.2cm]
            als Ergebnis den String
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{\symbol{34}a+b+c\symbol{34}}$
            \\[0.2cm]
            zurück.  In der Funktion $\texttt{determinant}$ wird die Funktion $\texttt{join}$ mit der Liste
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{[} a_{\texttt{1,L[1]}}, \cdots, a_{\texttt{n,L[n]}} \texttt{]}$ \quad und dem String \quad $\texttt{\symbol{34} * \symbol{34}}$
            \\[0.2cm]
            aufgerufen.  Daher berechnet der Aufruf von $\texttt{join}$ dann eine String-Darstellung des Produkts
            \\[0.2cm]
            \hspace*{1.3cm}
            $\ds\prod\limits_{i=1}^n a_{i,\sigma(i)}$,
            \\[0.2cm]
            wobei die Permutation $\sigma$ durch die Liste $\texttt{L}$ dargestellt wird.
      \end{enumerate}
\end{enumerate}
Für $n=4$ erhalten
...
Elemente und die Fakultätsfunktion wächst schneller als jede Potenz.  Für $n = 50$ haben wir beispielsweise
\\[0.2cm]
\hspace*{1.3cm}
$50! = 30\,414\,093\,201\,713\,378\,043\,612\,608\,166\,064\,768\,844\,377\,641\,568\,960\,512\,000\,000\,000\,000$
\\[0.2cm]
und diese Zahl hat 66 Stellen.  Genauer gilt die 
...
\\[0.2cm]
Diese Formel zeigt, dass die Fakultätsfunktion \blue{superexponentiell}\footnote{
  Eine Funktion $f:\mathbb{N} \rightarrow \mathbb{N}$ wächst  \blue{superexponentiell} genau dann, wenn sie
  schneller als jede Potenz wächst.  Als mathematische Formel schreibt sich dies wie folgt:
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall a \in \mathbb{R}: \Bigl(a > 1 \rightarrow \exists K\in \mathbb{N}: \forall n \in \mathbb{N}: 
   \bigl(K \leq n \rightarrow a^n < f(n)\bigr)\Bigr)
  $.
}
wächst.  Wir werden daher ein anderes
Verfahren zur Berechnung der Determinante entwickeln, bei dem nur etwa $n^3$ 
...
\item $\{ \tau \circ \mu \mid \mu \in \mathcal{S}_n \} \subseteq \mathcal{S}_n$.
      ...
      $\mathcal{S}_n$ unter dem relationalen Produkt abgeschlossen ist, was $\tau \circ \mu \in \mathcal{S}_n$
      für alle $\tau, \mu \in \mathcal{S}_n$ impliziert.
...
$
\\[0.2cm]
angeben.  Dabei bezeichnet $\vec{a}_i$ den $i$-ten \blue{Zeilenvektor} der Matrix $A = \bigl(a_{i,j}\bigr)_{i = 1,\cdots n \atop j=1,\cdots n}$, 
es gilt also
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
aus $A$ dadurch entsteht, dass wir die $k$-te Zeile von $A$ mit der $l$-ten Zeile von $A$ vertauschen.
\end{Satz}
...
& = & \ds
 \sum\limits_{\sigma \in \mathcal{S}_n} \texttt{sgn}(\sigma) \cdot a_{1,\tau\circ\sigma(1)} \cdot \mbox{$\dots$} \cdot a_{n,\tau\circ\sigma(n)} \\[0.5cm]
& = & \ds
...
\item Wir zeigen, dass für alle $\sigma \in \mathcal{S}_n$ die Gleichung
      ...
      \end{description}
\end{enumerate}
Nach diesen Vorüberlegungen führen wir jetzt den Beweis der eigentlichen Behauptung
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{enumerate}
\item \blue{Normiertheit} bedeutet
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{det}(\mathrm{E}_n) = 1$.
\item \blue{Alternierend} ist die Determinante, weil
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item \blue{Multilinear} ist die Determinante, weil
      \\[0.2cm]
      \hspace*{1.3cm}
...
hat gezeigt, dass die obigen Eigenschaften die Determinante bereits \red{vollständig} bestimmen: Jede Funktion
\\[0.2cm]
\hspace*{1.3cm}
...
Zeilen das Vorzeichen der Determinante umdreht.  Also haben wir einerseits
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{det}\bigl(\tau(A)\bigr) = \texttt{det}(A)$ \quad und andererseits \quad $\texttt{det}\bigl(\tau(A)\bigr) = -\texttt{det}(A)$.
\\[0.2cm]
Daraus folgt 
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{det}(A) = -\texttt{det}(A)$ \quad und also \quad
$2 \cdot \texttt{det}(A) = 0$.
\\[0.2cm]
Da wir annehmen\footnote{
...
Zahlen oder der Körper $\mathbb{C}$ der komplexen Zahlen ist, impliziert dies $\texttt{det}(A) = 0$.
\qed
...
\begin{Lemma} \label{lemma:la_det0}
...
\\[0.2cm]
Aus dieser Äquivalenz ergibt sich der folgende Satz:

\begin{Satz} \label{satz:12.17}
...
Es sei $B \in \mathbb{K}^{n \times n}$ und $\mathrm{ZE}$ sei eine Elementar-Matrix.
...
\begin{enumerate}
\item Fall: $\mathrm{ZE} = \mathrm{ZA}_n(k,l,\alpha)$ mit $k \not= l$. 
...
\item Fall: $\mathrm{ZE} = \mathrm{ZP}_n(k,l)$ mit $k \not = l$.

      Multiplizieren wir eine $n \times n$ Matrix $B$ von links mit
      $\mathrm{ZP}_n(k,l)$, so besteht der Effekt dieser Multiplikation darin, dass die $k$-te Zeile von $B$ mit der
      $l$-ten Zeile von $B$ vertauscht wird. Da das Vertauschen zweier Zeilen das Vorzeichen der Determinante
      umdreht, haben wir 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item[``$\Leftarrow$'']
     ...
     Nun sei $A$ nicht invertierbar.  Dann gilt nach Satz \ref{satz:12.17} $\mathtt{Kern}(A) \not= \{\vec{0}\}$.  Also gibt
     ...
     Spalten-Vektoren der Matrix $A$ linear abhängig und nach Lemma \ref{lemma:la_det0} folgt
     die Gleichung 
     \\[0.2cm]
     \hspace*{1.3cm}
     $\texttt{det}(A) = 0$. \qed
...
\item Aus Gründen der numerischen Stabilität des Verfahrens ist es sinnvoll, zunächst die Zeile
      ...
      in Abbildung \ref{fig:inverse.stlx-pivot} auf Seite \pageref{fig:inverse.stlx-pivot} gezeigt ist.  Wir
      hatten diese Funktion bereits bei der Berechnung des Inversen der Matrix $A$ benutzt.
...
\item Ansonsten ziehen wir nun das 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\ds\frac{\texttt{A[k][i]}}{\texttt{A[i][i]}}$-fache \quad der $i$-ten Zeile von der $k$-ten Zeile ab,
      \\[0.2cm]
...
\section{Reflexion}
\begin{enumerate}
\item Wie ist das Vorzeichen einer Permutation definiert?
\item Wie lautet die Homomorphie-Eigenschaft der Signum-Funktion?
\item Wie ist die Determinante einer Matrix definiert?
\item Welche Eigenschaften charakterisieren die Determinanten-Funktion?
\item Welcher Zusammenhang besteht zwischen der Invertierbarkeit einer Matrix und der Determinante der Matrix?
\item Wie kann die Determinante einer Matrix praktisch berechnet werden?
\item Warum gilt für beliebige $n \times n$ Matrizen die Gleichung
      $\texttt{det}(A) \cdot \texttt{det}(B) = \texttt{det}(A \cdot B)$?
\end{enumerate}
%%% Local Variables: 
%%% mode: latex
