...
\item Die Funktion $\texttt{determinant}$ erhält als Eingabe eine positive natürliche Zahl $\texttt{n}$ und
      liefert als Ergebnis einen String $\texttt{det}$ zurück, der den Inhalt der Leibniz-Formel wiedergibt. 
      Dabei wird die Definition der Determinante nach Leibniz unmittelbar umgesetzt:
      \begin{enumerate}
      \item Zunächst wird die Menge $\texttt{Sn}$ aller Permutationen der Zahlen von $1$ bis $\texttt{n}$ berechnet.
      \item Anschließend wird für jede Liste $\texttt{L}$ aus der Menge $\texttt{Sn}$ das Vorzeichen berechnet.
      ...
      \item Erklärungsbedürftig ist eventuell die Funktion $\texttt{join}$:  Diese wird im Allgemeinen mit
            zwei Argumenten aufgerufen in der Form
            \\[0.2cm]
            \hspace*{1.3cm}
            $\texttt{join}(L, s)$
            \\[0.2cm]
...
\\[0.2cm]
Diese Formel zeigt, dass die Fakultätsfunktion superexponentiell wächst.  Wir werden daher ein anderes
