...
\begin{Definition}[\blue{Signum}]
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $\blue{\mathtt{sgn}(R)} := (-1)^{\mathtt{card}(\mathtt{inv}(R))} = \left\{
   \begin{array}{ll}
     +1 & \mbox{falls  $\,\mathtt{card}\bigl(\mathtt{inv}(R)\bigr) \,\texttt{\%}\, 2 = 0$;} \\[0.2cm]     
     -1 & \mbox{falls  $\,\mathtt{card}\bigl(\mathtt{inv}(R)\bigr) \,\texttt{\%}\, 2 = 1$.} 
   \end{array}
   \right.
  $ %$
  \\[0.2cm]
...
Ist $\tau = \pair(k, k+1)$ eine elementare Transposition, so gilt
  \\[0.2cm]
  \hspace*{1.3cm}
  $\mathtt{inv}(\tau) = \bigl\{ \pair(k, k+1) \bigr\}$
  \\[0.2cm]
...
\item $\mathcal{S}_n \subseteq \{ \tau \circ \mu \mid \mu \in \mathcal{S}_n \}$.
      ...
      so dass $\sigma = \tau \circ \mu$ gilt.  Wir definieren diese Permutation $\mu$ als
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mu := \tau \circ \sigma$.
      \\[0.2cm]
...
$
\\[0.2cm]
angeben.  Dabei bezeichnet $\vec{a}_i$ den $i$-ten Zeilenvektor der Matrix $A = \bigl(a_{i,j}\bigr)_{1 \leq i \leq n \atop 1 \leq j \leq n}$, 
es gilt also
\\[0.2cm]
\hspace*{1.3cm}
...
wir die Eigenschaften der Vorzeichen-Funktion $\mathtt{sgn}$ tatsächlich benötigen.  
\eoxs
