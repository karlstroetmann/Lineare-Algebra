...
\item Definieren wir $\mathbb{Q}^+$ als die Menge der positiven rationalen Zahlen, also als
      ...
      \item $\frac{1}{q} \cdot q = 1$ \quad für alle $q \in \mathbb{Q}^+$,
            ...
            und damit ist die Zahl $\frac{1}{q}$ das \emph{\color{blue}Links-Inverse} der Zahl $q$ bezüglich der
            Multiplikation.
...
\proof
Es sei $a \in G$ beliebig und $b$ ein zu $a$ inverses Element.  Dann haben wir die folgende Kette von
Gleichungen:
\\[0.2cm]
\hspace*{1.3cm}
$
...
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{card}\bigl(f(M)\bigr) < n$ gelten muss.  \qed
...
\\[0.2cm]
\hspace*{1.3cm}
$\{ f(x_1), \cdots, f(x_n) \} = M$ \quad {\color{red}{\faBolt}}
\\[0.2cm] 
...
\begin{Definition}[Halb-Gruppe]
  Ein Paar $\langle G, \circ \rangle$ ist eine \emph{\color{blue}Halb-Gruppe}, falls gilt:
  \begin{enumerate}
  \item $G$ ist eine Menge,
...
\\[0.2cm]
eine Lösung $x$ bzw. $y$ haben, denn dann folgt die Behauptung aus Satz \ref{satz:solvable}.  Zunächst definieren wir für
...
als bijektive Funktion aufgefasst werden kann und das ist genau dann der Fall, wenn folgendes gilt:
\begin{enumerate}
...
\exercise
Zeigen Sie, dass $\mathcal{S}_3$ keine kommutative Gruppe ist.  Schreiben Sie dazu ein \textsc{SetlX}-Programm,
...
\item Wir definieren die Menge $G$ als
      ...
      \end{tabular}
      \\[0.2cm]
      Wollen wir zu gegebenen $x,y \in G$ den Wert $x \circ y$ mit Hilfe dieser Tabelle finden, so können
...
\\[0.2cm]
ist, und das ist genau dann der Fall, wenn $l_1 - l_2$ ein Vielfaches von $k$ ist, wenn also
